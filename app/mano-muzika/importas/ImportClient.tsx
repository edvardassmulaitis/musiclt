      ...
      {source === 'lastfm' && (
        <InputPanel disabled={!lastfmOk}
          ...
          <label className="mt-3 flex items-start gap-2.5 cursor-pointer select-none">
            ...
            <span className="text-[14px] leading-snug" style={{ color: 'var(--text-muted)' }}>
          ...
          {scan && lastfmFull && (
            <div className="mt-4 rounded-xl p-4" style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-default)' }}>
              ...
              <div className="text-[14px] font-bold mb-1.5">Ką kelti?</div>
              <div className="flex flex-wrap gap-2 mb-3">
                {([['artist', 'Atlikėjai'], ['album', 'Albumai'], ['track', 'Dainos']] as const).map(([k, lbl]) => {
                  ...
                  return <button key={k} onClick={() => setScKinds(s => { const n = new Set(s); n.has(k) ? n.delete(k) : n.add(k); return n })}
                    className="rounded-full px-3 py-1.5 text-[14px] font-bold" style={{ background: on ? 'rgba(249,115,22,0.15)' : 'var(--bg-surface)', border: `1px solid ${on ? 'rgba(249,115,22,0.5)' : 'var(--border-default)'}`, color: on ? 'var(--accent-orange)' : 'var(--text-muted)' }}>{on ? '✓ ' : ''}{lbl}</button>
                })}
              </div>
              <div className="text-[14px] font-bold mb-1.5">Kiek giliai?</div>
              <div className="flex flex-wrap gap-2 mb-3">
                {([['best', 'Tik mėgstami ir dažni'], ['all', 'Visa klausymų istorija']] as const).map(([k, lbl]) => (
                  <button key={k} onClick={() => setScHistory(k)} className="rounded-full px-3 py-1.5 text-[14px] font-bold" style={{ background: scHistory === k ? 'rgba(249,115,22,0.15)' : 'var(--bg-surface)', border: `1px solid ${scHistory === k ? 'rgba(249,115,22,0.5)' : 'var(--border-default)'}`, color: scHistory === k ? 'var(--accent-orange)' : 'var(--text-muted)' }}>{lbl}</button>
                ))}
              </div>
              <label className="flex items-center gap-2 text-[14px] mb-3" style={{ color: 'var(--text-muted)' }}>
                Min. klausymų skaičius:
                <input type="number" min={0} value={scMin} onChange={e => setScMin(Math.max(0, Number(e.target.value) || 0))} className="w-20 rounded-lg px-2 py-1 text-[14px] outline-none" style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-default)', color: 'var(--text-primary)' }} />
        ...
        <div className="mb-4 rounded-2xl p-5" style={{ background: 'linear-gradient(135deg, rgba(167,139,250,0.14), transparent)', border: '1px solid rgba(167,139,250,0.4)' }}>
          ...
          {job && (job.status === 'running' || job.status === 'queued') && job.batch_status !== 'reverted' && (
            <button onClick={() => revert({ jobId: job.id })} disabled={reverting}
              className="mt-3 text-[14px] font-bold underline disabled:opacity-40" style={{ color: 'var(--text-muted)' }}>
      ...
      {job && job.status === 'done' && !enqueued && (
        <div className="mb-4 rounded-2xl p-5" style={{ background: 'linear-gradient(135deg, rgba(52,211,153,0.14), transparent)', border: '1px solid rgba(52,211,153,0.4)' }}>
          ...
          {(job.batch_status === 'reverted' || reverted)
            ? <div className="mt-2 text-[14px] font-bold" style={{ color: 'var(--text-faint)' }}>Importas atšauktas.</div>
            : (job.batch_id && <button onClick={() => revert({ batchId: job.batch_id })} disabled={reverting}
                className="mt-2 text-[14px] font-bold underline disabled:opacity-40" style={{ color: 'var(--text-muted)' }}>
      ...
      {done && (
        <div className="rounded-2xl p-6 text-center" style={{ background: 'linear-gradient(135deg, rgba(52,211,153,0.14), transparent)', border: '1px solid rgba(52,211,153,0.4)' }}>
          ...
          <div className="mt-4 flex items-center justify-center gap-4">
            ...
            {reverted
              ? <span className="text-[14px] font-bold" style={{ color: 'var(--text-faint)' }}>Atšaukta</span>
              : (batchId && <button onClick={() => revert({ batchId })} disabled={reverting}
                  className="text-[14px] font-bold underline disabled:opacity-40" style={{ color: 'var(--text-muted)' }}>
