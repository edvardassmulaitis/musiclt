...
export function HomeListContent({ type, lane = 'lt', onOpenTrack, onOpenAlbum, onClose }: Props) {
  ...
  const chipCls = (active: boolean) =>
    `shrink-0 whitespace-nowrap rounded-full border px-2.5 py-1 font-['Outfit',sans-serif] text-[12.5px] font-bold transition-colors ${
      active
      ...
      >
        ...
        {overflow > 0 && (
          <button
            ...
            className="shrink-0 whitespace-nowrap rounded-full px-2.5 py-1 font-['Outfit',sans-serif] text-[12.5px] font-bold text-[var(--accent-orange)] transition-opacity hover:opacity-70"
          >
    ...
    <div style={{ minHeight: '58vh' }}>
      ...
      {!isEvents ? (
        <div className="mb-4 flex flex-col gap-2.5 sm:flex-row sm:items-center sm:gap-3">
          <div className="flex shrink-0 items-center gap-1.5">
            {([['new', type === 'upcoming' ? 'Artimiausi' : 'Naujausi'], ['liked', 'Populiariausi']] as const).map(([k, label]) => (
              <button key={k} type="button" onClick={() => setSort(k)}
                className={`rounded-full px-3 py-1.5 font-['Outfit',sans-serif] text-[13px] font-bold transition-colors ${sort === k ? 'bg-[var(--accent-orange)] text-white' : 'bg-[var(--bg-active)] text-[var(--text-muted)] hover:text-[var(--text-primary)]'}`}>
                {label}
              </button>
            ))}
          </div>
          {renderGenres()}
          {!loading && total > 0 && (
            <span className="shrink-0 font-['Outfit',sans-serif] text-[12.5px] font-bold text-[var(--text-faint)] sm:ml-auto">Iš viso: {total}</span>
          )}
        </div>
      ) : (
        <div className="mb-4 flex flex-col gap-2">
          <div className="hp-scroll flex min-w-0 items-center gap-1.5 overflow-x-auto pb-0.5 sm:flex-wrap sm:overflow-visible sm:pb-0">
            ...
            {!loading && <span className="ml-auto hidden shrink-0 font-['Outfit',sans-serif] text-[12.5px] font-bold text-[var(--text-faint)] sm:inline">Rasta: {shown.length}</span>}
          </div>
          {cities.length > 0 && (
            <div className="flex min-w-0 items-center gap-1.5">
              ...
              {venueOptions.length > 0 && (
                <select
                  ...
                  className={`shrink-0 max-w-[180px] truncate rounded-full border px-2.5 py-1 font-['Outfit',sans-serif] text-[12.5px] font-bold transition-colors ${activeVenue ? 'border-[var(--accent-orange)] bg-[var(--accent-orange)]/12 text-[var(--accent-orange)]' : 'border-[var(--border-default)] bg-[var(--bg-surface)] text-[var(--text-muted)]'}`}
                >
      ...
      {loading ? (
        ...
        <div className="py-12 text-center text-[14px] text-[var(--text-muted)]">Nieko nerasta</div>
      ) : (
        <>
          <div className={`grid gap-3 ${isTracks ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3' : 'grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5'}`}>
            {shown.map((it: any) => {
                ...
                return (
                  <Link key={it.id} href={`/renginiai/${it.slug}`} onClick={onClose} className="group block no-underline text-left">
                    ...
                    <div className="mt-2 px-0.5">
                      {validDate && (
                        <p className="m-0 mb-0.5 font-['Outfit',sans-serif] text-[12px] font-extrabold uppercase tracking-[0.03em] text-[var(--accent-orange)]">
                      ...
                      {venue && <p className="m-0 mt-1 truncate text-[12.5px] text-[var(--text-muted)]">{venue}</p>}
                    </div>
                  </Link>
                )
              }
              ...
              const metaRow = (
                <div className="mt-1 flex items-center gap-2">
                  <p className="m-0 min-w-0 flex-1 truncate text-[12.5px] text-[var(--text-muted)]">{artistName}</p>
                  <PopBar level={pop} />
                  {likes > 0 && (
                    <span className="flex shrink-0 items-center gap-0.5 text-[11.5px] font-bold text-[var(--text-muted)]">
              ...
              const dateBadge = rel.label ? (
                <span className={`absolute bottom-1.5 right-1.5 rounded px-1.5 py-0.5 font-['Outfit',sans-serif] text-[10px] font-bold backdrop-blur-sm ${rel.hot ? 'bg-[var(--accent-orange)] text-white' : 'bg-black/70 text-white'}`}>{rel.label}</span>
              ) : null
          ...
          {hasMore && (
            <div className="mt-5 flex justify-center">
              <button
                ...
                className="rounded-full border border-[var(--border-default)] bg-[var(--bg-surface)] px-5 py-2 font-['Outfit',sans-serif] text-[14px] font-bold text-[var(--text-primary)] transition-colors hover:border-[var(--accent-orange)] hover:text-[var(--accent-orange)] disabled:opacity-50"
              >
