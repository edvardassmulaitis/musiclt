...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (uid) {
      const { data } = await sb
        ...
        .eq('user_id', uid)
        .limit(3000)
      likedIds = Array.from(new Set((data || []).map((r: any) => Number(r.entity_id)).filter(Boolean)))
    }
  } catch { /* anon */ }

  // ── 2. Rekomenduojami atlikėjai (RPC) ────────────────────────────────────────
  ...
  if (!personalized) {
    try {
      ...
      recs = ((data || []) as any[])
        ...
        .map(a => ({ ...a, reason: (a.recent_score || 0) > 100 ? 'rising' : 'popular' }))
    } catch { /* ignore */ }
  }

  const recIds = recs.map(r => r.artist_id)
  const recById = new Map(recs.map(r => [r.artist_id, r]))
  const matchIds = Array.from(new Set([...likedIds, ...recIds])) // koncertams + temoms
  const topRecIds = recIds.slice(0, 16) // leidiniams — tik stipriausi

  const queues: Record<string, RecItem[]> = { artist: [], release: [], event: [], topic: [] }

  // ── 3a. Atlikėjų kortelės ────────────────────────────────────────────────────
  for (const r of recs) {
    queues.artist.push({
      key: `artist-${r.artist_id}`,
      kind: 'artist',
      title: r.name,
      subtitle: r.country || null,
      image: r.cover_image_url,
      href: `/atlikejai/${r.slug || r.artist_id}`,
      date: null,
      badge: REASON_BADGE[r.reason] || REASON_BADGE.similar,
      reason: r.reason,
      artist: { id: r.artist_id, name: r.name, slug: r.slug },
    })
  }

  // ── 3b. Nauji leidiniai (tracks + albums) iš rekomenduojamų atlikėjų ─────────
  if (topRecIds.length) {
    try {
      const { data } = await sb
        .from('tracks')
  ...
  // ── 3c. Artėjantys koncertai (event_artists ∩ pamėgti/rekomenduojami) ────────
  if (matchIds.length) {
    try {
      ...
      if (eventIds.length) {
        ...
        for (const ev of (data || []) as any[]) {
          queues.event.push({
            ...
            href: `/renginiai/${ev.slug}`, date: ev.start_date, badge: 'Koncertas tau',
          })
        }
      }
    } catch { /* ignore */ }
    ...
    try {
      ...
      for (const d of (data || []) as any[]) {
        ...
        queues.topic.push({
          ...
          href: `/diskusijos/${d.slug}`, date: d.last_comment_at || d.created_at, badge: 'Tema tau',
  ...
  const items = weave(queues, limit)
  return NextResponse.json({ items, personalized, recommendedCount: recs.length })
}
