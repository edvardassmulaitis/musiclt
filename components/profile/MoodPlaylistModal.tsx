      ...
      <div className="w-full max-w-[920px] max-h-[92vh] overflow-hidden rounded-2xl flex flex-col"
        ...
        <div className="flex items-center justify-between px-4 sm:px-5 py-3 border-b" style={{ borderColor: 'var(--border-subtle)' }}>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-[15px] font-black" style={{ fontFamily: "'Outfit', sans-serif" }}>Nuotaikos dainos <span className="font-bold text-[14px]" style={{ color: 'var(--text-faint)' }}>{songs.length}</span></h2>
          </div>
          ...
          <div className="p-4 sm:p-5 flex flex-col min-h-0">
            <div className="relative w-full rounded-xl overflow-hidden" style={{ aspectRatio: '16 / 9', background: '#000' }}>
              {curId ? (
                ...
                <div className="absolute inset-0 flex items-center justify-center text-center px-4 text-[14.5px]" style={{ color: 'rgba(255,255,255,0.6)' }}>Šiai dainai nėra grojamo įrašo.</div>
              )}
            </div>
            {current && (
              <div className="mt-3">
                <div className="font-extrabold text-[16px] leading-tight" style={{ fontFamily: "'Outfit', sans-serif" }}>{current.title}</div>
                {curArtist && <Link href={`/atlikejai/${curArtist.slug}`} className="text-[14px] font-semibold hover:underline" style={{ color: 'var(--text-secondary)' }}>{curArtist.name}</Link>}
              </div>
            )}
          </div>
                ...
                return (
                  <li key={s.id}>
                    <button onClick={() => hasVid && setCurrent(s)} disabled={!hasVid}
                      ...
                      <span className="w-5 shrink-0 text-center text-[13px] font-black tabular-nums" style={{ color: active ? 'var(--accent-orange)' : 'var(--text-faint)' }}>{i + 1}</span>
                      ...
                      <div className="min-w-0 flex-1"><div className="truncate text-[14.5px] font-bold">{s.title}</div><div className="truncate text-[12.5px]" style={{ color: 'var(--text-muted)' }}>{s.artist?.name || 'Daina'}</div></div>
                      {active && hasVid && (
