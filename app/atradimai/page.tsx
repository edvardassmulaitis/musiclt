...
//
// „Atradimai" — vientisas horizontalių row'ų bendruomenės hub'as.
//
// Tvarka:
//   1. Plona antraštė
//   2. Kas vyksta · Pokalbiai (2 platūs, žemesni widget'ai)
//   3. Dienos daina — homepage stiliaus juosta (winner + siūlomos)
//   4. Diskusijos (su atlikėjo vizualu + paskutinio komentatoriaus avataru)
//   5. Naujausi įrašai (visi tipai)
//   6. Pagal kategorijas: Narių topai · Recenzijos · Kūryba · Vertimai · Straipsniai
//   7. Apačioje: Nauji nariai · Boombox
//
// VIZUALAI + DEDUP: įrašų row'ai naudoja /api/atradimai/feed — jis išsprendžia
// viršelį iš prikabintų dainų/albumų/atlikėjų (dauguma migruotų įrašų neturi
// cover_image_url) ir dedup'ina po vieną įrašą per autorių (nefloodina vienas).
//
// Duomenys: /api/atradimai/feed · /api/diskusijos/recent · /api/dienos-daina/* ·
//   /api/atradimai/active-members · <ActivityWidget/> <ShoutboxWidget/>
          ...
          {items.map(d => (
            <Link key={d.id} href={`/diskusijos/${d.slug}`} className="group flex w-[300px] shrink-0 snap-start gap-3 rounded-2xl border border-[var(--border-default)] bg-[var(--bg-surface)] p-3 no-underline transition-all hover:-translate-y-0.5 hover:border-[rgba(139,92,246,0.5)]">
              ...
              <div className="flex min-w-0 flex-1 flex-col">
                {d.artist_name && <span className="mb-0.5 truncate text-[10px] font-extrabold uppercase tracking-[0.06em] text-[var(--accent-purple,#8b5cf6)]" style={{ color: '#8b5cf6' }}>{d.artist_name}</span>}
