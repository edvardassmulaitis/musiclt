...
export default async function StudioFans({ searchParams }: { searchParams: Promise<{ a?: string }> }) {
  ...
  const [totalR, weekR, monthR, emailR, listR] = await Promise.all([
  ...
  return (
    <div className="max-w-2xl">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[['Viso fanų', total], ['+7 d.', weekR.count || 0], ['+30 d.', monthR.count || 0], ['Sutiko gauti el. laiškus', emailR.count || 0]].map(([l, v]) => (
