...
type HeroSlide = {
  ...
  chartTops?: TopEntry[]
  // daily_winner koliažas — dienos dainos kandidatai (laimėtojas didesnis).
  // Naudojam vietoj vieno low-quality YouTube thumbnail'o.
...
function HeroV2Card({ slide, dk }: { slide: HeroSlide; dk: boolean }) {
  ...
  // Dienos daina su kandidatų koliažu (vietoj vieno low-quality YT thumb).
...
/* Dienos daina — kandidatų koliažas (kaip top40/top30), laimėtojas didžiausias.
   Naudoja aukštos kokybės cover'ius (ne low-quality YouTube thumbnail'ą). */
function HeroDailyCard({ slide }: { slide: HeroSlide }) {
  ...
  return (
    ...
    >
      ...
      {/* RIGHT collage — laimėtojas didelis + kandidatai */}
...
export default function HomeClient({ initialLatest, initialHero }: { initialLatest?: InitialLatest; initialHero?: InitialHero }) {
  ...
  // Dienos dainos laimėjimo dienos kandidatai (koliažui). Fetch'inam pagal laimėtojo datą.
  const [dailyDayParts, setDailyDayParts] = useState<any[]>([])
  ...
  useEffect(() => {
    ...
    core.push(fetch('/api/dienos-daina/winners?limit=7').then(r => r.json()).then(d => {
      const winners = d.winners || []
      setDailyWinners(winners)
      // Laimėjimo dienos kandidatai → koliažas (aukštos kokybės cover'iai vietoj YT thumb).
      const w0 = winners[0]
      if (w0?.date) {
        fetch(`/api/dienos-daina/day?date=${encodeURIComponent(w0.date)}`)
          .then(r => r.json())
          .then(dd => setDailyDayParts(Array.isArray(dd.participants) ? dd.participants : []))
          .catch(() => {})
      }
    }).catch(() => {}))
  ...
  useEffect(() => {
    ...
    if (dailyWinners.length > 0) {
      ...
      if (tr && ageDays <= 2.5) {
        ...
        // Koliažas — aukštos kokybės cover'iai (ne low-quality YT thumb). Laimėtojas
        // pirmas (didžiausias), po jo tos dienos kandidatai, o jei jų mažai —
        // papildom paskutinių dienų laimėtojais (kad koliažas visada užsipildytų).
        const coverOf = (t: any): string | null =>
          t?.cover_url || t?.artists?.cover_image_url ||
          (extractYouTubeId(t?.video_url || null) ? `https://img.youtube.com/vi/${extractYouTubeId(t?.video_url || null)}/hqdefault.jpg` : null)
        ...
        pushTrack(tr, true)                                                   // laimėtojas
        for (const p of (dailyDayParts || [])) pushTrack(p?.tracks, false)    // tos dienos kandidatai
        for (const w2 of dailyWinners) { if (collage.length >= 5) break; pushTrack(w2?.tracks, false) } // fill
        dailySlides.push({
    ...
    tryReady.current()
  }, [news, events, newsSongs, ltTop, worldTop, ltTopDate, worldTopDate, heroPosts, heroEvents, discoveries, recordings, dailyWinners, dailyNomsCount, dailyDayParts, vertaConcerts, feedOverrides, feedCustom, feedBlocked])
