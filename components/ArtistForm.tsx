...
const SocialIcons: Record<string, React.ReactNode> = {
  ...
  instagram: (
    <svg viewBox="0 0 24 24" className="w-4 h-4" fill="url(#igGrad)" aria-hidden="true">
      <defs>
        <radialGradient id="igGrad" cx="0.3" cy="1" r="1.2">
          <stop offset="0" stopColor="#FFC107" />
          <stop offset="0.45" stopColor="#FF1744" />
          <stop offset="0.85" stopColor="#A020F0" />
        </radialGradient>
      </defs>
...
export type ArtistFormData = {
  ...
  gender: 'male'|'female'|''
  facebook: string; youtube: string; tiktok: string; instagram: string
    ...
    <div className="mt-2">
      <button type="button" onClick={() => setOpen(true)}
        ...
        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-colors flex items-center justify-center">
          <span className="text-white text-[12px] font-medium opacity-0 group-hover:opacity-100 transition-opacity">Keisti poziciją</span>
        </div>
        <div className="absolute bottom-1 right-1 bg-black/60 text-white text-[12px] font-mono px-1 py-0.5 rounded">
      ...
      <div className="bg-white rounded-xl shadow-2xl overflow-hidden max-w-lg w-full mx-4 max-h-[100vh] sm:max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="px-4 py-3 border-b flex items-center justify-between">
          <div>
            ...
            <p className="text-[14px] text-gray-400">Tempk nuotrauką ir naudok scroll zoom'ui</p>
          </div>
        ...
        <div className="px-4 py-3 border-t flex items-center gap-3">
          <span className="text-[14px] text-gray-400">Zoom</span>
          ...
          <button type="button" onClick={() => { setXPct(50); setYPct(20); setZoom(1); emit(50, 20, 1) }}
            className="text-[14px] text-gray-400 hover:text-gray-600 ml-2">Reset</button>
        </div>
      </div>
    </div>
    </MobileFullscreenPortal>
        ...
        <div key={i} className="flex items-center gap-1.5 bg-[var(--bg-elevated)] rounded-lg px-2 py-2 flex-wrap">
          ...
          {onToggle && (
            <button type="button" onClick={()=>onToggle(i)}
              ...
              className="px-1.5 py-0.5 ml-0.5 rounded text-[12px] font-semibold text-[var(--text-muted)] hover:text-blue-600 hover:bg-blue-50 border border-[var(--text-faint)] transition-colors whitespace-nowrap">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            {inactiveCount > 0 && (
              <button type="button" onClick={activateAll}
                ...
                className="bg-amber-50 hover:bg-amber-100 text-amber-700 border border-amber-200 text-[12px] font-semibold px-1.5 py-0.5 rounded-full transition-colors">
          ...
          {photos.map((p, i) => (
            <div key={i} className="relative group"
              ...
              <div className={`aspect-square rounded-lg overflow-hidden bg-[var(--bg-elevated)] cursor-pointer ring-2 transition-all ${dragOverIdx === i ? 'ring-blue-400 scale-95' : p.is_active === false ? 'ring-amber-300' : 'ring-transparent'}`}
                ...
                {p.is_active === false && (
                  <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 text-center pointer-events-none">
                    <span className="bg-amber-500/90 text-white text-[12px] font-bold uppercase px-1.5 py-0.5 rounded">neaktyvi</span>
                  </div>
                )}
              </div>
              ...
              <button type="button" onClick={e => { e.stopPropagation(); toggleActive(i) }}
                ...
                className={`absolute top-0.5 right-5 w-4 h-4 rounded-full text-[12px] font-bold flex items-center justify-center transition-all ${p.is_active === false ? 'bg-amber-400 hover:bg-amber-500 text-white opacity-100' : 'bg-black/50 hover:bg-amber-500 text-white opacity-0 group-hover:opacity-100'}`}>
                {p.is_active === false ? '✓' : '✕'}
              </button>
              {onSetAvatar && (
                <button type="button" onClick={e => { e.stopPropagation(); onSetAvatar(p.url) }}
                  ...
                  className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full text-[12px] flex items-center justify-center transition-all ${currentAvatar === p.url ? 'bg-blue-500 text-white opacity-100' : 'bg-black/50 hover:bg-blue-500 text-white opacity-0 group-hover:opacity-100'}`}>
                  {currentAvatar === p.url ? '★' : '☆'}
                </button>
              )}
              {onSetHero && (
                <button type="button" onClick={e => { e.stopPropagation(); onSetHero(p.url) }}
                  ...
                  className={`absolute top-0.5 left-6 w-5 h-5 rounded text-[12px] font-bold flex items-center justify-center transition-all ${currentHero === p.url ? 'bg-purple-500 text-white opacity-100' : 'bg-black/50 hover:bg-purple-500 text-white opacity-0 group-hover:opacity-100'}`}>
                  H
                </button>
              )}
              {p.author && (
                <div className="absolute inset-x-0 bottom-0 rounded-b-lg bg-gradient-to-t from-black/75 to-transparent px-1.5 pt-4 pb-1 pointer-events-none">
                  <p className="text-white/90 text-[12px] leading-tight truncate">© {p.author.split(' · ')[0]}</p>
                </div>
              )}
              <button type="button" onClick={e => { e.stopPropagation(); setEditMetaIdx(editMetaIdx === i ? null : i) }}
                className="absolute bottom-0.5 right-0.5 w-5 h-5 bg-black/50 hover:bg-blue-500 text-white rounded text-[12px] font-bold flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all">©</button>
            </div>
          ))}
        </div>
      )}
...
export default function ArtistForm({ initialData, artistId, onSubmit, backHref, title, submitLabel, onChange, hideButtons, onRegisterSubmit }: Props) {
  ...
  useEffect(() => {
    ...
    const next = {
      ...
      members: initialData.members ?? prev.members,
        ...
        <div className="p-2.5 sm:p-3 sm:pt-4 sm:pb-4 border-b lg:border-b-0 lg:border-r border-[var(--border-subtle)] space-y-2 sm:space-y-3">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <div>
              ...
              <Sel value={form.genre} onChange={(v:string)=>set('genre',v)} required>
                <option value="">Pasirinkite...</option>
