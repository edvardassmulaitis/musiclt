...
async function fetchGroupFullData(wikiTitle: string): Promise<Partial<MemberFullData> & { avatar: string }> {
  ...
  try {
    ...
    // Wikidata
    const ppRes = await fetch(`https://en.wikipedia.org/w/api.php?action=query&titles=${encodeURIComponent(wikiTitle)}&prop=pageprops&format=json&origin=*&redirects=1`)
...
function WikipediaImportCore({ onImport, initialSearch }: Props) {
  ...
  const go = async (overrideUrl?: string) => {
    ...
    try {
      ...
      const ppRes = await fetch(`https://en.wikipedia.org/w/api.php?action=query&titles=${encodeURIComponent(s)}&prop=pageprops&format=json&origin=*&redirects=1`)
      const wdId: string = (Object.values((await ppRes.json()).query?.pages || {})[0] as any)?.pageprops?.wikibase_item || ''
        ...
        <div className="border border-gray-200 rounded-lg overflow-hidden">
          ...
          <div className="flex border-b border-gray-200 bg-gray-50">
            {([
              ...
              >
                <span className={`w-5 h-5 rounded text-[12px] font-bold flex items-center justify-center leading-none ${tab.badgeCls}`}>{tab.badge}</span>
                <span className="hidden sm:inline">{tab.label}</span>
                {tab.loading
                  ? <span className="text-gray-400 animate-spin text-xs">⟳</span>
                  : tab.count > 0
                  ? <span className="text-[12px] bg-gray-200 text-gray-600 rounded-full px-1.5">{tab.count}</span>
                  : null
                }
              </button>
            ))}
                        ...
                        <span className="text-gray-500">
                          {currentMembers.map((m, i) => (
                            <span key={m.wikiTitle}>
                              ...
                              {m.existingId && <span className="text-green-400 ml-0.5 text-[12px]">✓</span>}
                            </span>
                          ))}
                        </span>
                      )}
              ...
              <div>
                ...
                {(preview as any)?._descEditing && (
                  <div className="mt-2">
                    ...
                    />
                    <div className="text-[12px] text-gray-400 mt-0.5">Pastraipos skiriamos tuščia eilute. Paspauskit kitur kad išsaugoti.</div>
                  </div>
                )}
              </div>
            ) : null}
