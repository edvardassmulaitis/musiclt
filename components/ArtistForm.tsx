...
function AvatarUploadCompact({ value, onChange, onOriginalSaved }: {
  value: string
  onChange: (url: string) => void
  onOriginalSaved?: (squareUrl: string, origUrl: string) => void  // ✅ abu URL kartu
}) {
  const fileRef = useRef<HTMLInputElement>(null)
  const [uploading, setUploading] = useState(false)
  const [error, setError] = useState('')
  ...
  // ✅ PATAISYTA: abu uploadai baigiami, tada kviečiamas vienas callback su abiem URL
  const handleCropped = async ({ square, original }: CropResult) => {
    setCropSrc(null)
    setUploading(true); setError('')
    try {
      const squareUrl = await uploadBlob(square, 'avatar-square.jpg')
      const origUrl = await uploadBlob(original, 'avatar-original.jpg')
      // Kviečiame vieną callback su abiem URL — parent atnaujina avatar + photos vienu metu
      if (onOriginalSaved) {
        onOriginalSaved(squareUrl, origUrl)
      } else {
        onChange(squareUrl)
      }
      setUrlInput(squareUrl)
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="grid grid-cols-2 gap-5">
            ...
            <div className="space-y-5">
              ...
              <Card title="Profilio nuotrauka">
                {/* ✅ PATAISYTA: onOriginalSaved gauna abu URL ir atnaujina vienu setState */}
                <AvatarUploadCompact
                  value={form.avatar}
                  onChange={setAvatar}
                  onOriginalSaved={(squareUrl, origUrl) => {
                    const currentPhotos = formRef.current.photos
                    const newPhotos = currentPhotos.find((p: any) => p.url === origUrl)
                      ? currentPhotos
                      : [{ url: origUrl }, ...currentPhotos]
                    const next = { ...formRef.current, avatar: squareUrl, photos: newPhotos }
                    setForm(next)
                    if (onChange && next.name) onChange(next)
                  }}
                />
