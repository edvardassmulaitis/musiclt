...
function AdminTopInner() {
  ...
  useEffect(() => {
    if (status !== 'authenticated' || !isAdmin) return
    loadSuggestions()
  }, [status, isAdmin, topType, suggestionStatus]) // eslint-disable-line
  ...
  const reviewSuggestion = async (id: number, newStatus: 'approved' | 'rejected') => {
    ...
    if (res.ok) {
      ...
      loadSuggestions()
      if (newStatus === 'approved') loadEntries()
    } else {
          ...
          <div className="space-y-4">
            ...
            {loadingSuggestions ? (
              ...
              <div className="space-y-1.5">
                {suggestions.map(s => (
                  <div key={s.id} className="flex items-center gap-3 px-3 py-2.5 bg-white border border-gray-100 rounded-xl shadow-sm">
                    <div className="w-9 h-9 rounded-lg bg-gray-100 flex items-center justify-center text-sm flex-shrink-0">♪</div>
                    <div className="flex-1 min-w-0">
