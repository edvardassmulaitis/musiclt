    ...
    <div>
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2">
        <div className={`${card} p-4`}>
          <div className="mb-2.5 flex items-center gap-2.5 font-[Outfit,sans-serif] text-[16px] font-bold text-[var(--text-primary)]">
      ...
      <div className={`${card} mt-3 p-3.5`}>
        <div className="mb-2.5 flex items-center"><b className="font-[Outfit,sans-serif] text-sm font-bold text-[var(--text-primary)]">Galerija</b><span className="ml-2 text-[14px] text-[var(--text-muted)]">užvesk → hero / profilis · arba įkelk naują</span></div>
        <div className="flex gap-2.5 overflow-x-auto pb-1">
          <label className="grid h-[92px] w-[124px] shrink-0 cursor-pointer place-items-center rounded-[10px] border border-dashed border-[var(--border-strong)] text-center text-[14px] text-[var(--text-muted)]" onClick={() => flash(true, 'Nuotraukų įkėlimas — netrukus')}>
          ...
          {photos.map((p) => {
            ...
            return (
              <div key={p.id} className="group relative h-[92px] w-[124px] shrink-0 overflow-hidden rounded-[10px] border border-[var(--border-subtle)] bg-[var(--bg-surface)]">
                ...
                {(isHero || isProf) && <span className={`absolute left-1.5 top-1.5 rounded-full px-1.5 py-0.5 text-[12px] font-bold text-white ${isHero ? 'bg-[#f97316]' : 'bg-black/60'}`}>{isHero ? 'Hero' : 'Profilis'}</span>}
        ...
        <div>
          ...
          {songs.map((s) => (
            <div key={s.id} className={`mb-2 flex items-center gap-3 rounded-xl border p-2.5 ${s.is_pinned ? 'border-[rgba(249,115,22,0.35)] bg-[rgba(249,115,22,0.05)]' : 'border-[var(--border-subtle)] bg-[var(--bg-elevated)]'}`}>
              ...
              <div className="min-w-0 flex-1"><b className="block truncate text-[14px] font-semibold text-[var(--text-primary)]">{s.title}</b><small className="text-[12px] text-[var(--text-muted)]">{s.is_pinned ? 'Prisegta · ' : ''}{s.video_uploaded_at ? new Date(s.video_uploaded_at).toLocaleDateString('lt-LT') : (s.release_year || '—')}</small></div>
              {s.state === 'eligible' && <button onClick={() => suggest(s)} disabled={busy === 's' + s.id} className="inline-flex items-center gap-1.5 rounded-full bg-[#f97316] px-3 py-1.5 text-[12px] font-bold text-white font-[Outfit,sans-serif]">{I.trophy} Top 40</button>}
              {s.state === 'in' && <span className="rounded-full bg-[rgba(34,197,94,.14)] px-3 py-1.5 text-[12px] font-bold text-[var(--accent-green)]">Topе · {s.weeks} sav.</span>}
              {s.state === 'pending' && <span className="rounded-full bg-[rgba(251,191,36,.13)] px-3 py-1.5 text-[12px] font-bold text-[#fbbf24]">Pasiūlyta</span>}
              {s.state === 'wait' && <span className="rounded-full bg-[rgba(251,191,36,.13)] px-3 py-1.5 text-[12px] font-bold text-[#fbbf24]">Palauk</span>}
              {s.state === 'too_old' && <span className="rounded-full border border-[var(--border-subtle)] bg-[var(--bg-hover)] px-3 py-1.5 text-[12px] font-bold text-[var(--text-faint)]">Per sena</span>}
            </div>
          ))}
          <p className="mt-1.5 text-[12px] text-[var(--text-faint)]">Prisegtos rodomos viršuje, atlikėjo playeryje. Į Top 40 — tik per 3 mėn. įkeltos.</p>
        </div>
        <div>
          ...
          {events.map((e) => { const d = new Date(e.start_date); return (
            <div key={e.id} className="mb-2 flex items-center gap-3 rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-elevated)] p-2.5">
              <div className="w-[42px] shrink-0 rounded-lg border border-[var(--border-default)] bg-[var(--bg-surface)] py-1 text-center"><b className="block font-[Outfit,sans-serif] text-base font-extrabold leading-none">{d.getDate()}</b><span className="text-[12px] uppercase text-[var(--text-muted)]">{MONTHS[d.getMonth()]}</span></div>
              <div className="min-w-0 flex-1"><b className="block truncate text-[14px] text-[var(--text-primary)]">{e.venue_name || e.title}</b><small className="text-[12px] text-[var(--text-muted)]">{e.city || e.title}</small></div>
            </div>
          )})}
      ...
      {/* APPEARANCE */}
      <div className={gt}>Išvaizda <span className="ml-2 normal-case tracking-normal font-medium text-[14px] text-[var(--text-muted)]">— tavo viešos anketos</span></div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {SECTIONS.map((s) => { const on = !hidden.includes(s.key); return (
              <button key={s.key} onClick={() => { const nh = on ? [...hidden, s.key] : hidden.filter((x) => x !== s.key); setHidden(nh); saveAppearance({ hidden_sections: nh }) }}
                className={`rounded-full border px-2.5 py-1 text-[14px] font-semibold font-[Outfit,sans-serif] ${on ? 'border-[rgba(249,115,22,0.3)] bg-[rgba(249,115,22,0.13)] text-[#f97316]' : 'border-[var(--border-default)] text-[var(--text-muted)]'}`}>{s.label}</button>
            )})}
          </div>
        </div>
      </div>
      ...
      {/* COMPLETENESS */}
      <div className="mt-4 flex items-center gap-4 rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-elevated)] px-4 py-3 text-[14px] text-[var(--text-secondary)]">
...
function More({ href, title, sub }: { href: string; title: string; sub: string }) {
  return (
    <a href={href} className="flex items-center gap-3 rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-elevated)] px-3.5 py-3 transition hover:border-[rgba(249,115,22,0.35)]">
      <div><b className="font-[Outfit,sans-serif] text-[14px] font-semibold text-[var(--text-primary)]">{title}</b><span className="block text-[14px] text-[var(--text-muted)]">{sub}</span></div>
    </a>
  )
}
