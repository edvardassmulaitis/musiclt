  ...
  return (
    <div>
      ...
      <div className={`${card} mt-3 p-3.5`}>
        <div className="mb-2.5 flex items-center"><b className="font-[Outfit,sans-serif] text-sm font-bold text-[var(--text-primary)]">Galerija</b><span className="ml-2 text-[11px] text-[var(--text-muted)]">užvesk → hero / profilis · arba įkelk naują</span></div>
        <div className="flex gap-2.5 overflow-x-auto pb-1">
          <label className="grid h-[92px] w-[124px] shrink-0 cursor-pointer place-items-center rounded-[10px] border border-dashed border-[var(--border-strong)] text-center text-[11px] text-[var(--text-muted)]" onClick={() => flash(true, 'Nuotraukų įkėlimas — netrukus')}>
        ...
        <div>
          ...
          {songs.map((s) => (
            <div key={s.id} className={`mb-2 flex items-center gap-3 rounded-xl border p-2.5 ${s.is_pinned ? 'border-[rgba(249,115,22,0.35)] bg-[rgba(249,115,22,0.05)]' : 'border-[var(--border-subtle)] bg-[var(--bg-elevated)]'}`}>
              ...
              {s.state === 'too_old' && <span className="rounded-full border border-[var(--border-subtle)] bg-[var(--bg-hover)] px-3 py-1.5 text-[10.5px] font-bold text-[var(--text-faint)]">Per sena</span>}
            </div>
          ))}
          <p className="mt-1.5 text-[10.5px] text-[var(--text-faint)]">Prisegtos rodomos viršuje, atlikėjo playeryje. Į Top 40 — tik per 3 mėn. įkeltos.</p>
        </div>
        <div>
          <div className={gt}>Renginiai <span className="ml-auto inline-flex items-center gap-1 text-[12px] font-semibold normal-case tracking-normal text-[var(--text-faint)]">Pridėti — netrukus</span></div>
          {events.map((e) => { const d = new Date(e.start_date); return (
            <div key={e.id} className="mb-2 flex items-center gap-3 rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-elevated)] p-2.5">
              <div className="w-[42px] shrink-0 rounded-lg border border-[var(--border-default)] bg-[var(--bg-surface)] py-1 text-center"><b className="block font-[Outfit,sans-serif] text-base font-extrabold leading-none">{d.getDate()}</b><span className="text-[9px] uppercase text-[var(--text-muted)]">{MONTHS[d.getMonth()]}</span></div>
              <div className="min-w-0 flex-1"><b className="block truncate text-[13px] text-[var(--text-primary)]">{e.venue_name || e.title}</b><small className="text-[10.5px] text-[var(--text-muted)]">{e.city || e.title}</small></div>
            </div>
          )})}
      ...
      {/* APPEARANCE */}
      <div className={gt}>Išvaizda <span className="ml-2 normal-case tracking-normal font-medium text-[11px] text-[var(--text-muted)]">— tavo viešos anketos</span></div>
      <div className="grid gap-3.5 sm:grid-cols-3">
        ...
        <div className={`${card} p-4`}>
          <h4 className="mb-2.5 font-[Outfit,sans-serif] text-[13px] font-bold text-[var(--text-primary)]">Akcento spalva</h4>
        ...
        <div className={`${card} p-4`}>
          <h4 className="mb-2.5 font-[Outfit,sans-serif] text-[13px] font-bold text-[var(--text-primary)]">Ką rodyti anketoje</h4>
...
function More({ href, title, sub }: { href: string; title: string; sub: string }) {
  return (
    <a href={href} className="flex items-center gap-3 rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-elevated)] px-3.5 py-3 transition hover:border-[rgba(249,115,22,0.35)]">
      <div><b className="font-[Outfit,sans-serif] text-[13px] font-semibold text-[var(--text-primary)]">{title}</b><span className="block text-[11px] text-[var(--text-muted)]">{sub}</span></div>
    </a>
  )
}
