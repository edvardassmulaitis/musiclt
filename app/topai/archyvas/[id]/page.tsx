      ...
      <div className="mt-6 rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-surface)] overflow-hidden">
        {main.length === 0 ? (
          ...
          return (
            <div
              key={e.id}
              className={`flex items-center gap-3 px-4 py-3 ${i < main.length - 1 ? 'border-b border-[var(--border-subtle)]' : ''}`}
            >
              <span
              ...
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-sm text-[var(--text-primary)] truncate">{track?.title ?? '—'}</p>
                <p className="text-xs text-[var(--text-muted)] truncate">{artist?.name ?? '—'}</p>
              </div>
              {e.total_votes ? (
