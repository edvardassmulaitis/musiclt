    ...
    <div className="py-1.5">
      ...
      {details && (
        <div className="flex items-center gap-3 mt-0.5">
          ...
          <span className="text-[12px] text-[var(--text-faint)] leading-tight">{details}</span>
        </div>
      )}
    </div>
  )
    ...
    <div className={`rounded-xl border border-[var(--border-default)] bg-[var(--card-bg)] p-4 ${className}`}>
      <div className="flex items-baseline justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          {formulaTag && (
            <span className="text-[12px] px-1.5 py-0.5 rounded bg-[var(--bg-elevated)] text-[var(--text-faint)] font-medium uppercase">
              {formulaTag}
            </span>
          )}
        </div>
