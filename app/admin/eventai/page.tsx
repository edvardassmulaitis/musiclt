    ...
    <div style={{ padding: '24px 16px', maxWidth: 1200, margin: '0 auto' }}>
      ...
      {(Object.keys(grouped) as Array<keyof typeof grouped>).map(g => (
        <section key={g as string} style={{ marginBottom: 20 }}>
          <h2 style={{ fontSize: 14, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.06em', color: '#888', margin: '0 0 8px 4px' }}>
            ...
            {grouped[g as string].map(meta => {
              ...
              return (
                ...
                >
                  ...
                  <code style={{ fontSize: 14, color: '#555', background: '#f5f5f5', padding: '2px 6px', borderRadius: 4 }}>
                    {meta.type}
                  </code>
                  <div style={{ fontSize: 14, color: '#888' }}>{meta.source}</div>
                </button>
              )
            })}
          </div>
          ...
          rows.map(row => (
            <div key={row.id} style={{
              ...
              <button
                ...
                style={{ flexShrink: 0, padding: '4px 10px', fontSize: 14, color: '#ef4444', background: 'none', border: '1px solid #fecaca', borderRadius: 6, cursor: 'pointer' }}
              >
...
function Stat({ label, value }: { label: string; value: string | number }) {
  return (
    <div>
      <div style={{ fontSize: 14, color: '#888', textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 700 }}>{label}</div>
