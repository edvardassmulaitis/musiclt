        ...
        {playing && !isBlocked ? (
          ...
          <a href={`https://www.youtube.com/watch?v=${vid}`} target="_blank" rel="noopener noreferrer"
            ...
            <span className="relative z-10 flex flex-col items-center gap-2 px-6 text-center text-white">
              ...
              <span className="text-[14.5px] font-semibold">Žiūrėti YouTube'e</span>
            </span>
          </a>
        ) : (
          <button type="button" onClick={() => vid && play(activeIdx)} aria-label="Paleisti" className="group absolute inset-0 block cursor-pointer overflow-hidden border-0 p-0">
            ...
            <span className="absolute inset-x-0 bottom-0 px-3 pb-3 pr-16 text-left">
              ...
              <span className="block truncate text-[13px] text-white/80">{cur.artistName}</span>
            </span>
          </button>
        )}
      </div>
        ...
        {items.map((s, i) => {
          ...
          return (
            <li key={s.id}>
              ...
              >
                <span className={['w-4 shrink-0 text-center font-["Outfit",sans-serif] text-[13px] font-bold tabular-nums', isActive ? 'text-[var(--accent-orange)]' : 'text-[var(--text-faint,var(--text-muted))]'].join(' ')} aria-hidden>{i + 1}</span>
                <div className="flex min-w-0 flex-1 flex-col items-start">
                  <span className={['w-full truncate font-["Outfit",sans-serif] text-[14.5px] font-bold leading-tight', isActive ? 'text-[var(--accent-orange)]' : 'text-[var(--text-primary)]'].join(' ')}>{s.title}</span>
                  <span className="w-full truncate text-[12.5px] text-[var(--text-muted)]">{s.artistName}</span>
                </div>
      ...
      {!openMobile && (
        <button onClick={() => play(activeIdx)} className="flex w-full items-center gap-3 rounded-2xl border border-[var(--border-default)] bg-[var(--card-bg)] p-2.5 text-left lg:hidden">
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="block truncate text-[13px] text-[var(--text-muted)]">{cur.artistName} ir kt. · {items.length} dainos</span>
          </span>
