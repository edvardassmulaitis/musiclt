...
export default function AdminLyricsSuggestions() {
  ...
  return (
    <div className="mx-auto max-w-[900px] px-4 py-6">
      <div className="mb-4">
        <Link href="/admin" className="text-[12px] font-bold text-[var(--text-muted)] no-underline hover:text-[var(--accent-orange)]">← Admin</Link>
        <h1 className="mt-1 font-['Outfit',sans-serif] text-[22px] font-extrabold text-[var(--text-primary)]">📝 Dainų tekstų pasiūlymai</h1>
        <p className="mt-1 text-[13px] text-[var(--text-muted)]">Vartotojų pasiūlyti tekstai. Patvirtinus — įrašoma į dainą.</p>
      </div>
      ...
      <div className="mb-4 flex items-center gap-2">
        {(['pending', 'approved', 'rejected'] as const).map(t => (
          <button
            ...
            onClick={() => setTab(t)}
            className={[
              "rounded-full px-3 py-1 font-['Outfit',sans-serif] text-[12px] font-bold transition-colors",
        ...
        <button onClick={load} className="ml-auto rounded-full border border-[var(--border-subtle)] px-3 py-1 text-[12px] font-bold text-[var(--text-muted)] hover:text-[var(--text-primary)]">↻ Atnaujinti</button>
      </div>

      {msg && <div className="mb-3 rounded-lg bg-[rgba(249,115,22,0.1)] px-3 py-2 text-[12px] font-semibold text-[var(--accent-orange)]">{msg}</div>}

      {loading ? (
        <div className="py-10 text-center text-[13px] text-[var(--text-muted)]">Kraunama…</div>
      ) : items.length === 0 ? (
        <div className="rounded-xl border border-dashed border-[var(--border-subtle)] py-12 text-center text-[13px] text-[var(--text-faint)]">
          Nėra pasiūlymų.
        </div>
      ) : (
          ...
          {items.map(it => (
            <div key={it.id} className="rounded-xl border border-[var(--border-subtle)] bg-[var(--card-bg)] p-4">
              <div className="mb-2 flex flex-wrap items-baseline gap-x-2 gap-y-1">
                ...
                {it.artist_name && <span className="text-[13px] text-[var(--text-muted)]">— {it.artist_name}</span>}
                {it.track_has_lyrics && (
                  <span className="rounded-full bg-[rgba(234,179,8,0.15)] px-2 py-0.5 text-[10px] font-bold text-[#b45309]">jau turi tekstą — pakeis</span>
                )}
              </div>
              ...
              <pre className="mb-3 max-h-[320px] overflow-auto whitespace-pre-wrap rounded-lg bg-[var(--bg-elevated)] p-3 font-['DM_Sans',sans-serif] text-[12.5px] leading-[1.6] text-[var(--text-secondary)]">
                {it.lyrics}
              </pre>
              {tab === 'pending' && (
                <div className="flex items-center gap-2">
                  <button
                    ...
                    disabled={busy === it.id}
                    className="rounded-lg bg-[var(--accent-orange)] px-3.5 py-1.5 font-['Outfit',sans-serif] text-[12px] font-extrabold text-white disabled:opacity-50"
                  >
                  ...
                  <button
                    ...
                    disabled={busy === it.id}
                    className="rounded-lg border border-[var(--border-subtle)] px-3.5 py-1.5 font-['Outfit',sans-serif] text-[12px] font-bold text-[var(--text-muted)] hover:text-[var(--text-primary)] disabled:opacity-50"
                  >
                    Atmesti
                  </button>
                </div>
