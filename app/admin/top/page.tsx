...
function TrendBadge({ curr, prev }: { curr: number | null; prev: number | null }) {
  if (prev === null || curr === null)
    return <span className="text-[12px] font-bold px-1.5 py-0.5 rounded bg-amber-50 text-amber-600 border border-amber-200">NEW</span>
                    ...
                    <div key={c.track_id}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                          {c.reasons.map((r, i) => (
                            <span key={i} className="text-[12px] font-semibold px-1.5 py-0.5 rounded bg-indigo-50 text-indigo-600 border border-indigo-100">
                              {r}
                            </span>
                          ))}
                        </div>
                        {(c.history.last_top_week || c.history.last_suggested_at) && (
                          <p className={`text-[12px] mt-1 ${c.penalty ? 'text-amber-600 font-semibold' : 'text-gray-400'}`}>
