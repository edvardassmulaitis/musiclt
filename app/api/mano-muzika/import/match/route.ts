...
import { matchItems, type RawItems } from '@/lib/music-import'
...
export async function POST(req: NextRequest) {
  ...
  const raw: RawItems = {
    artists: Array.isArray(body.artists) ? body.artists.filter((x: any) => x?.name).slice(0, 200) : [],
    tracks: Array.isArray(body.tracks) ? body.tracks.filter((x: any) => x?.artist && x?.title).slice(0, 200) : [],
    albums: Array.isArray(body.albums) ? body.albums.filter((x: any) => x?.artist && x?.title).slice(0, 200) : [],
  }
