...
function CategoryBadge({ category }: { category: string | null }) {
  ...
  return (
    <span
      className="inline-flex items-center gap-1.5 rounded-md px-2 py-1 text-[12px] font-bold uppercase tracking-wide backdrop-blur-sm"
...
export default function NewsCard({
  ...
  if (variant === 'hero') {
    return (
      ...
      >
        {item.image ? (
          <img src={item.image} alt={item.title} loading="eager" fetchPriority="high" decoding="async" className="absolute inset-0 h-full w-full object-cover object-top transition-transform duration-500 group-hover:scale-105" />
        ) : (
        ...
        <div className="relative z-10 flex flex-col gap-3 p-6 sm:p-8">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {item.isLT && (
              <span className="rounded-full bg-white/15 px-2 py-0.5 text-[12px] font-bold uppercase tracking-wide text-white backdrop-blur">🇱🇹 Lietuva</span>
            )}
          </div>
    ...
    return (
      ...
      >
        <div className="relative aspect-[16/10] w-full shrink-0 overflow-hidden bg-[var(--bg-hover)]">
          {item.image ? (
            <img src={item.image} alt={item.title} loading="eager" decoding="async" className="h-full w-full object-cover object-top transition-transform duration-500 group-hover:scale-[1.04]" />
          ) : (
          ...
          <div className="absolute left-3 top-3 flex flex-wrap gap-1.5">
            <CategoryBadge category={item.category} />
            {item.isLT && (
              <span className="rounded-md bg-black/55 px-2 py-1 text-[12px] font-bold uppercase tracking-wide text-white backdrop-blur-sm">🇱🇹 Lietuva</span>
            )}
          </div>
        </div>
        <div className="flex flex-1 flex-col gap-2 p-4">
          ...
          <div className="mt-auto flex items-center justify-between gap-2 text-[14px] text-[var(--text-faint)]">
    ...
    return (
      <Link href={item.href} className="group flex items-center gap-3 rounded-xl p-2 transition-colors hover:bg-[var(--bg-surface-hover,rgba(125,125,125,0.07))]">
        <div className="h-14 w-14 shrink-0 overflow-hidden rounded-lg border border-[var(--border-default)]">
          {item.image ? (
            <img src={item.image} alt={item.title} loading="lazy" decoding="async" className="h-full w-full object-cover object-top" />
          ) : (
            <Placeholder accent={catAccent} />
          )}
        </div>
  ...
  return (
    ...
    >
      <div className="relative aspect-[16/9] w-full shrink-0 overflow-hidden bg-[var(--bg-hover)]">
        {item.image ? (
          <img src={item.image} alt={item.title} loading="lazy" decoding="async" className="h-full w-full object-cover object-top transition-transform duration-500 group-hover:scale-[1.04]" />
        ) : (
          <Placeholder accent={catAccent} />
        )}
