...
// Format date: "2007 m. spalio 17 d."
...
export default function AlbumPageClient({ album, artist, tracks, otherAlbums, similarAlbums, likes, relatedNews = [] }: Props) {
  ...
  // Like button — small, top-right corner style
  const LikeBtn = () => (
    <button
  ...
  // Tracklist shared between desktop right column and mobile combined card
  const TrackList = () => (
    <>
  ...
  return (
    <div style={{ background: T.bg, color: T.text, fontFamily: "'DM Sans',system-ui,sans-serif", WebkitFontSmoothing: 'antialiased', minHeight: '100vh' }}>
      ...
      {/* ══════════════════════════════════════════
          DESKTOP layout: sidebar | tracklist
      ══════════════════════════════════════════ */}
      <div className="ab-desktop" style={{ maxWidth: 1400, margin: '0 auto', padding: '14px 20px 60px', display: 'grid', gridTemplateColumns: '340px 1fr', gap: 14, alignItems: 'start' }}>
        ...
        <div style={{ position: 'sticky', top: 14, display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {/* Album info */}
          <div style={card}>
            <div style={{ background: T.coverAreaBg, padding: '16px', display: 'flex', gap: 14, alignItems: 'center', opacity: loaded ? 1 : 0, transform: loaded ? 'none' : 'translateY(6px)', transition: 'opacity .4s, transform .4s', position: 'relative' }}>
              {/* Like btn — top right corner */}
              <div style={{ position: 'absolute', top: 12, right: 12 }}><LikeBtn /></div>
              {/* Cover — bigger: 110px */}
              <div style={{ flexShrink: 0, width: 110, height: 110, borderRadius: 12, overflow: 'hidden', boxShadow: dk ? '0 10px 32px rgba(0,0,0,.7)' : '0 6px 24px rgba(0,0,0,.2)', background: T.coverBg }}>
                {album.cover_image_url
      ...
      {/* ══════════════════════════════════════════
          ...
          YouTube + tracklist merged into one card
      ══════════════════════════════════════════ */}
      <div className="ab-mobile" style={{ display: 'none', padding: '12px 14px 56px', flexDirection: 'column', gap: 12 }}>

        {/* Album info card */}
        <div style={card}>
          <div style={{ background: T.coverAreaBg, padding: '14px', display: 'flex', gap: 12, alignItems: 'center', opacity: loaded ? 1 : 0, transition: 'opacity .4s', position: 'relative' }}>
            {/* Like — top right */}
            <div style={{ position: 'absolute', top: 10, right: 12 }}><LikeBtn /></div>
            {/* Cover bigger on mobile too */}
            <div style={{ flexShrink: 0, width: 100, height: 100, borderRadius: 12, overflow: 'hidden', boxShadow: dk ? '0 8px 24px rgba(0,0,0,.65)' : '0 5px 18px rgba(0,0,0,.18)', background: T.coverBg }}>
              {album.cover_image_url
                ? <img src={album.cover_image_url} alt={album.title} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
            ...
            <div style={{ flex: 1, minWidth: 0, paddingRight: 52 }}>
              ...
              <h1 style={{ fontFamily: 'Outfit, sans-serif', fontSize: 18, fontWeight: 900, lineHeight: 1.1, letterSpacing: '-.02em', color: dk ? '#fff' : '#0f1a2e', margin: '0 0 4px', wordBreak: 'break-word' }}>{album.title}</h1>
              <Link href={`/atlikejai/${artist.slug}`} style={{ fontSize: 14, fontWeight: 700, color: '#f97316', textDecoration: 'none', display: 'block', marginBottom: 3 }}
                onMouseEnter={e => (e.currentTarget.style.opacity = '.75')}
                onMouseLeave={e => (e.currentTarget.style.opacity = '1')}>{artist.name}
              </Link>
              {dateStr && <div style={{ fontSize: 11, color: T.textMuted }}>{dateStr}</div>}
        ...
        {/* Combined YouTube + Tracklist card */}
        <div style={card}>
          {/* Player header */}
          {currentTrack && (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '9px 14px 8px', borderBottom: `1px solid ${T.subBdr}`, gap: 8 }}>
              <div style={{ minWidth: 0 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: T.text, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{currentTrack.title}</div>
                <div style={{ fontSize: 10, color: T.textMuted }}>{artist.name}</div>
              </div>
              <span style={{ fontSize: 9, fontWeight: 700, color: T.textMuted, textTransform: 'uppercase', letterSpacing: '.06em', flexShrink: 0 }}>YouTube</span>
            </div>
          )}
          ...
          {/* Divider + tracklist header */}
          <div style={{ ...cardHead, borderBottom: `1px solid ${T.subBdr}`, borderTop: `1px solid ${T.subBdr}` }}>
