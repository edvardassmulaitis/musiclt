  ...
  return (
    <Link href={`/skelbimai/skelbimas/${listing.id}`} className="sk-card group flex w-full flex-col overflow-hidden no-underline">
      ...
      <div className="relative aspect-video overflow-hidden">
        ...
        {sub && (
          <span className="absolute left-2 top-2 rounded px-1.5 py-0.5 font-['Outfit',sans-serif] text-[9px] font-extrabold uppercase tracking-[0.05em] text-white backdrop-blur-sm" style={{ background: accent }}>
            {sub}
          </span>
        )}
        {listing.is_promoted && (
          <span className="absolute right-2 top-2 rounded px-1.5 py-0.5 font-['Outfit',sans-serif] text-[9px] font-extrabold uppercase tracking-[0.05em] text-white" style={{ background: 'rgba(249,115,22,0.92)' }}>★</span>
        )}
      </div>

      {/* Turinys */}
      <div className="flex flex-1 flex-col p-3">
        <p className="m-0 line-clamp-2 font-['Outfit',sans-serif] text-[13.5px] font-extrabold leading-snug text-[var(--text-primary)] transition-colors group-hover:text-[var(--accent-orange)]">{listing.title}</p>
        ...
        <div className="mt-auto flex items-center gap-2 pt-2.5">
          {artist?.cover_image_url ? (
            <>
              ...
              <span className="min-w-0 flex-1 truncate text-[11px] font-semibold text-[var(--text-secondary)]">{artist.name}</span>
            </>
          ) : (
            <span className="min-w-0 flex-1 truncate text-[11px] text-[var(--text-muted)]">{bottomLeft}</span>
          )}
          <span className="shrink-0 text-[9.5px] text-[var(--text-faint)]">{rightLabel}</span>
        </div>
      </div>
