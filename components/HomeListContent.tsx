...
export function HomeListContent({ type, lane = 'lt', onOpenTrack, onOpenAlbum, onClose }: Props) {
  ...
  const fetchPage = useCallback(async (offset: number): Promise<{ items: any[]; total: number; genres?: Facet[] }> => {
    if (isEvents) {
      const r = await fetch('/api/events?limit=200&compact=1&homepage=1').then(res => res.json()).catch(() => ({ events: [] }))
          ...
          <div className="flex shrink-0 items-center gap-1.5">
            {([['new', type === 'upcoming' ? 'Artimiausi' : 'Naujausi'], ['liked', 'Populiariausi']] as const).map(([k, label]) => (
              <button key={k} type="button" onClick={() => setSort(k)}
                className={`rounded-full px-3 py-1.5 font-['Outfit',sans-serif] text-[14px] font-bold transition-colors ${sort === k ? 'bg-[var(--accent-orange)] text-white' : 'bg-[var(--bg-active)] text-[var(--text-muted)] hover:text-[var(--text-primary)]'}`}>
                {label}
              </button>
            ))}
          </div>
