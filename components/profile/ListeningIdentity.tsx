      ...
      <div className="max-w-[1200px] mx-auto px-5 sm:px-8">
        <div className="relative rounded-3xl overflow-hidden bg-gradient-to-br from-[#111822] via-[#0a1018] to-[#080c12] border border-white/[.06]">
          ...
          <div className="absolute top-5 sm:top-8 left-5 sm:left-10 right-5 sm:right-10 flex items-center justify-between">
            <div className="text-[10px] sm:text-[11px] font-extrabold uppercase tracking-[0.22em] text-[#f97316]" style={{ fontFamily: "'Outfit', sans-serif" }}>
              Klausymo identitetas
            </div>
            <div className="text-[10px] text-[#5e7290] uppercase tracking-wider font-bold" style={{ fontFamily: "'Outfit', sans-serif" }}>
              {username}
            </div>
          </div>
          ...
          <div className="relative grid grid-cols-1 lg:grid-cols-[1.1fr_1fr] gap-8 lg:gap-12 p-6 sm:p-10 lg:p-14 pt-16 sm:pt-20 lg:pt-24">
            ...
            <div className="flex flex-col justify-center">
              {top3.length > 0 ? (
                <>
                  <div className="text-[11px] font-extrabold uppercase tracking-widest text-[#5e7290] mb-4" style={{ fontFamily: "'Outfit', sans-serif" }}>
                  ...
                  {favoriteStyles.length > 3 && (
                    <div className="mt-6 flex flex-wrap gap-1.5">
                      {favoriteStyles.slice(3, 12).map((s) => (
                        <span key={s.legacy_style_id} className="text-[10px] sm:text-xs text-[#5e7290] font-semibold" style={{ fontFamily: "'Outfit', sans-serif" }}>
                          {s.style_name}{' '}<span className="text-[#334058]">·</span>
                        </span>
                      ))}
                      {favoriteStyles.length > 12 && (
                        <span className="text-[10px] text-[#334058] font-bold">+{favoriteStyles.length - 12} dar</span>
                      )}
                    </div>
                  )}
                </>
            ...
            {hasMeter && (
              <div className="flex flex-col justify-end">
                <div className="text-[11px] font-extrabold uppercase tracking-widest text-[#5e7290] mb-3 lg:text-right" style={{ fontFamily: "'Outfit', sans-serif" }}>
                    ...
                    return (
                      <div key={s.legacy_id} className="flex flex-col items-center flex-1 min-w-0 group">
                        ...
                        <div className="w-full text-center mt-2">
                          <div className="text-[9px] sm:text-[10px] font-bold text-[#dde8f8] truncate" style={{ fontFamily: "'Outfit', sans-serif" }}>
                            {s.name}
                          </div>
                          <div className="text-[9px] text-[#5e7290] font-mono">{(s.percent || 0).toFixed(0)}%</div>
                        </div>
                      </div>
                    )
                  })}
