                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {resolved && <span className="text-[14px] px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-700">✓ Sutvarkyta</span>}
                      ...
                      {evs.length > 0 ? (
                        <div className="flex items-center gap-1.5 flex-wrap mt-1.5">
                          {evs.slice(0, 4).map((ev, i) => (
                            <span key={i} className={`inline-flex items-center gap-1 text-[14px] px-1.5 py-0.5 rounded border ${ev.upcoming ? 'bg-cyan-50 border-cyan-200 text-cyan-800' : 'bg-gray-50 border-gray-200 text-gray-500'}`} title={ev.upcoming ? 'Būsimas' : 'Praėjęs'}>
                              {ev.is_festival ? '🎪' : '🎫'}
                          ...
                          {evs.length > 4 && <span className="text-[14px] text-gray-400">+{evs.length - 4}</span>}
                        </div>
                      ) : (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className="text-[14px] px-1.5 py-0.5 rounded bg-gray-100 text-gray-500">{SRC_LABEL[it.source] || it.source}</span>
                      {it.kind_hint && <span className="text-[14px] px-1.5 py-0.5 rounded bg-gray-50 text-gray-400">{it.kind_hint}</span>}
                      {it.source === 'import' && !!it.followers && it.followers > 0 && (
                        <span className="text-[14px] px-1.5 py-0.5 rounded bg-violet-100 text-violet-700" title="Tiek narių laukia šios muzikos — sutvarkius automatiškai atsiras jų profilyje">👤 {it.followers} laukia</span>
                      )}
                      {resolved && it.matched_type && (
                        <span className="text-[14px] px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-700">✓ {it.matched_type}</span>
                      )}
