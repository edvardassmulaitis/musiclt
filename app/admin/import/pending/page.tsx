...
async function getPendingAlbums(): Promise<PendingEntry[]> {
...
async function getPendingTracks(): Promise<PendingEntry[]> {
  const sb = createAdminClient()
...
async function PendingTable() {
  const [albums, tracks] = await Promise.all([getPendingAlbums(), getPendingTracks()])
