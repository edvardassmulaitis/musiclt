...
function MiniStat({ href, label, doneN, totalN, pct, color }: {
  ...
  return (
    ...
    >
      ...
      <span className="text-right text-[10px] font-semibold tabular-nums text-[var(--text-secondary)]">
        {pct.toFixed(1)}%
      </span>
    </Link>
  )
...
export default function AdminMigrationProgress() {
  ...
  if (error) {
    return (
      <Link
        ...
        className="block rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-[12px] text-red-700 hover:bg-red-100"
      >
  ...
  if (!s) {
    return (
      <div className="rounded-xl border border-[var(--input-border)] bg-[var(--bg-surface)] px-3 py-2 text-[12px] text-[var(--text-muted)]">
    ...
    <div className="overflow-hidden rounded-xl border border-[var(--input-border)] bg-[var(--bg-surface)]">
      ...
      >
        <div className="flex items-baseline gap-2">
          ...
          <h2 className="font-['Outfit',sans-serif] text-[12.5px] font-extrabold uppercase tracking-[0.1em] text-[var(--text-primary)]">
        ...
        <span className="text-[11px] text-music-blue">Detali migracija →</span>
      </Link>
      ...
      {s.stats_coverage.pct < 50 && (
        <div className="border-t border-[var(--border-subtle)] bg-amber-50 px-3 py-1.5 text-[10.5px] text-amber-800">
