...
//
// V18i: nario savitvarkos modalas — leidžia REDAGUOTI profilio info, kuri
// rodoma profilyje: avatarą (kolekcija + įkėlimas), viršelį, aprašymą (bio),
// šūkį, miestą, užsiėmimą, gimimo metus, mėgstamas knygas ir nario nuotraukas.
// Saugo per PUT /api/profile (admin client, service role). Tik savininkui.

import { useEffect, useRef, useState } from 'react'
import { createPortal } from 'react-dom'
import { useRouter } from 'next/navigation'

// Nauja muzikinė avatarų kolekcija (SVG, public/avatars/).
export const AVATAR_COLLECTION = Array.from({ length: 12 }, (_, i) => `/avatars/av-${String(i + 1).padStart(2, '0')}.svg`)
...
export function ProfileEditModal({ profile, onClose }: { profile: any; onClose: () => void }) {
  ...
  const [occupation, setOccupation] = useState<string>(profile.legacy_occupation || '')
  ...
  const onPickAvatar = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploading('avatar'); setErr(null)
    try { const u = await uploadFile(f); if (u) setAvatarUrl(u) }
    catch (x: any) { setErr(x.message) } finally { setUploading(null) }
  }
  const onPickCover = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const f = e.target.files?.[0]; if (!f) return
    setUploading('cover'); setErr(null)
    try { const u = await uploadFile(f); if (u) setCoverUrl(u) }
    catch (x: any) { setErr(x.message) } finally { setUploading(null) }
  }
  const onPickPhoto = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      for (const f of files.slice(0, 8)) { const u = await uploadFile(f); if (u) setPhotos((p) => [...p, { url: u }]) }
    } catch (x: any) { setErr(x.message) } finally { setUploading(null) }
  }
  ...
  async function save() {
    ...
    const body: Record<string, any> = {
      ...
      legacy_occupation: occupation.trim() || null,
    ...
    <div onClick={onClose} className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-6 backdrop-blur-md" style={{ background: 'rgba(0,0,0,0.65)' }}>
      <div onClick={(e) => e.stopPropagation()} className="relative w-full max-w-2xl max-h-[94vh] sm:max-h-[90vh] flex flex-col rounded-t-2xl sm:rounded-2xl overflow-hidden"
        ...
        <div className="flex-1 overflow-y-auto px-5 sm:px-6 py-5 flex flex-col gap-5">
          ...
          <div>
            ...
            <div className="grid grid-cols-6 gap-2">
              {AVATAR_COLLECTION.map((u) => {
                ...
                return (
                  <button key={u} type="button" onClick={() => setAvatarUrl(u)}
          ...
          {/* Cover */}
          <div>
            <span className={label} style={labelStyle}>Viršelio nuotrauka (fonui)</span>
            <div className="flex items-center gap-3">
              ...
              {coverUrl && (
                <button type="button" onClick={() => setCoverUrl('')} className="text-[12px] font-bold" style={{ color: 'var(--text-muted)' }}>Pašalinti</button>
              )}
          ...
          {/* Šūkis */}
          <div>
            <span className={label} style={labelStyle}>Šūkis (rodomas po vardu)</span>
            <input className={field} style={fieldStyle} value={signature} maxLength={160} onChange={(e) => setSignature(e.target.value)} placeholder="Trumpa eilutė apie tave…" />
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <input className={field} style={fieldStyle} value={city} maxLength={60} onChange={(e) => setCity(e.target.value)} placeholder="pvz. Vilnius" />
            </div>
            <div>
            ...
            <div>
              <span className={label} style={labelStyle}>Užsiėmimas</span>
              <input className={field} style={fieldStyle} value={occupation} maxLength={80} onChange={(e) => setOccupation(e.target.value)} placeholder="pvz. Studijuoju" />
            </div>
            <div>
          ...
          <div>
            ...
            <div className="grid grid-cols-4 sm:grid-cols-6 gap-2">
              {photos.map((p, i) => (
                <div key={i} className="relative aspect-square rounded-lg overflow-hidden group" style={{ border: '1px solid var(--border-subtle)' }}>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
