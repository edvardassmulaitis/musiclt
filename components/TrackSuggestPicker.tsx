      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-3 py-2 space-y-2 bg-gray-50">
        ...
        <SectionTight title="🔍 YouTube paieška" count={null}>
          ...
          {aiMentions.length > 0 && (() => {
            ...
            return (
              <div className="flex flex-wrap gap-1 mb-1 px-0.5">
                <span className="text-[12px] text-gray-500 self-center">📰 Naujienoje paminėtos:</span>
                {unmatched.slice(0, 6).map((m, i) => (
                  <button
                    ...
                    className="px-1.5 py-0.5 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-full text-[12px] font-medium border border-dashed border-blue-300">
                    {m.title}
                  </button>
                ))}
              </div>
          ...
          {ytResults.length === 0 && !ytSearching && (
            <p className="text-[12px] text-gray-400 italic px-1">Nieko nerasta. Pakeisk paieškos užklausą.</p>
          )}
          {ytSearching && (
...
function SectionTight({ title, count, children }: { title: string; count: number | null; children: React.ReactNode }) {
  return (
    <div className="bg-white rounded-lg border border-gray-200">
      <div className="px-2 py-1 text-[12px] font-semibold text-gray-600 uppercase tracking-wide bg-gray-50 border-b border-gray-100">
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200">
      <button
        type="button"
        onClick={() => setOpen(o => !o)}
        className="w-full px-2 py-1 text-[12px] font-semibold text-gray-600 uppercase tracking-wide bg-gray-50 border-b border-gray-100 flex items-center justify-between">
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-[12px] text-gray-500 truncate">{subtitle}</div>
      </div>
      <Badge type={badge} />
    </div>
  )
