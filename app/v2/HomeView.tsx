...
// Bendras homepage vaizdas — naudojamas IR „/" (main), IR „/v2" (preview alias).
// Route config (revalidate/metadata) — atitinkamuose route failuose (app/page.tsx,
// app/v2/page.tsx). Šis modulis NĖRA route (todėl jį galima importuoti be Next
// „importing a page" klaidos).
...
/* ─────────────── Muzikos pool'as (/v2) ───────────────
   PER-LANE DB užklausa (kaip /dainos) → pilna 90 d. LT aprėptis. /api/home/list
   NETINKA — jo bendras 250-kandidatų limitas per visus lane'us apkerpa LT iki
   ~2 sav. (LT 15–90 d. dainų ten VISAI nėra). Užklausa LENGVA (be album_tracks
   nested join'o, kuris Vercel'yje timeout'indavo → dingdavo dainos).
   Reissue filtras pagal release_year (gaudo senas dainas su nesenu YT re-upload'u,
   pvz. Depeche Mode 2001). Song + per-atlikėją dedup. Albumams — recency langas +
   isReleased (ne ateities, ne seni). Alternaciją/rikiavimą daro klientas. */
...
const ALBUM_POOL_DAYS = 120  // albumų recency langas
...
async function fetchLaneTracks(sb: any, lane: 'lt' | 'world', sinceIso: string, nowIso: string, currentYear: number): Promise<any[]> {
  ...
  const valid = ((data || []) as any[]).filter((t) => {
    ...
    // Reissue filtras (pagal metus): sena daina su nesenu YT re-upload'u → NErodom.
...
async function fetchLaneAlbums(sb: any, lane: 'lt' | 'world', currentYear: number, albumSinceMs: number, nowMs: number): Promise<any[]> {
  ...
  return ((data || []) as any[]).filter((a) => {
    ...
    const d = albumDateMs(a)
    return d > 0 && d >= albumSinceMs && d <= nowMs // išleistas, per recency langą, ne ateities
  })
}
...
function buildHeroSlides(input: {
  ...
  verta.filter((c: any) => !!c.image).slice(0, 1).forEach((c: any) => {
    ...
    const dateStr = d && !isNaN(d.getTime()) ? `${d.getDate()} ${MONTHS_LT[d.getMonth()]}` : ''
    const dateFull = d && !isNaN(d.getTime()) ? `${d.getFullYear()} m. ${MONTHS_FULL_LT[d.getMonth()]} ${d.getDate()} d.` : ''
    const title = c.isFestival ? (c.festivalName || c.artist) : c.artist
    slides.push({
      type: 'verta', chip: 'VERTA KELIONĖS', chipBg: '#0891b2',
      title: sanitizeTitle(title || ''),
      subtitle: [c.venue, dateStr].filter(Boolean).join(' · '),
      metaLine: [c.venue, dateFull].filter(Boolean).join(' · '),
      excerpt: c.why ? sanitizeTitle(c.why) : '',
      ticketUrl: c.ticketUrl || null,
      ctaLabel: 'Apie kelionę',
      href: `/verta-keliones#vk-${c.slug || c.id}`,
      bgImg: c.image, fresh24: false,
      artist: null,
    })
  })
