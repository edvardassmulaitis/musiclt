              ...
              return (
                <div key={it.id} className={`bg-white border rounded-xl p-3 sm:p-4 ${it.reviewed ? 'border-gray-200 opacity-70' : 'border-gray-300'}`}>
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {it.hidden && <span className="text-[12px] px-1.5 py-0.5 rounded bg-red-100 text-red-600 font-semibold">narys paslėptas</span>}
                        {it.is_deleted && <span className="text-[12px] px-1.5 py-0.5 rounded bg-red-100 text-red-700 font-semibold">🗑 ištrintas</span>}
                        {it.reviewed && <span className="text-[12px] px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-700 font-semibold">sutvarkyta</span>}
                        {it.featured && <span className="text-[12px] px-1.5 py-0.5 rounded bg-amber-100 text-amber-700 font-semibold" title={it.featured_until ? `iki ${new Date(it.featured_until).toLocaleString('lt-LT')}` : ''}>★ verta dėmesio</span>}
                        {it.home_hero && <span className="text-[12px] px-1.5 py-0.5 rounded bg-orange-100 text-orange-700 font-semibold">🏠 hero</span>}
                      </div>
                    ...
                    <span className={`shrink-0 text-[12px] px-2 py-1 rounded-lg font-semibold ${onHome ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-500'}`}>
                  ...
                  <div className="flex flex-wrap items-center gap-2 mt-3">
                    ...
                    {it.post_type === 'topas' && it.topas && (
                      <div className="flex items-center gap-2">
                        <span className={`text-[12px] px-2 py-1 rounded-lg font-semibold ${fmtBadge[it.topas.format].c}`}>
                  ...
                  {it.post_type === 'topas' && it.topas && it.topas.unmatched > 0 && it.topas.format === 'new' && (
                    <div className="mt-2 text-[13px] text-amber-700 bg-amber-50 border border-amber-100 rounded-lg px-2.5 py-1.5">
