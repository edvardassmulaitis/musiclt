...
const labelCls = 'mb-1 block text-[14px] font-semibold text-[var(--text-muted)]'
    ...
    <div className="sm:col-span-2 rounded-lg border border-[var(--border-default)] bg-[var(--bg-elevated)]/40 p-3">
      ...
      {d.event_id ? (
        <div className="flex items-center gap-2 rounded-lg border border-[#ec4899]/40 bg-[#ec4899]/5 px-3 py-2">
          ...
          <button type="button" className="text-[14px] font-semibold text-[var(--text-muted)] hover:text-red-500" onClick={unlink}>Atsieti</button>
        </div>
      ) : (
        <>
          {suggest.length > 0 && (
...
function groupControl(lineup: LineupItem[], value: string, onChange: (v: string) => void, tagValue: string, onTag: (v: string) => void) {
  return (
    <span className="inline-flex flex-wrap items-center gap-1.5">
      <select className="rounded border border-[var(--input-border)] bg-[var(--input-bg)] px-1.5 py-1 text-[14px] text-[var(--input-text)]" value={value} onChange={(e) => onChange(e.target.value)}>
      ...
      {value === 't:__custom' && (
        <input className="w-28 rounded border border-[var(--input-border)] bg-[var(--input-bg)] px-1.5 py-1 text-[14px] text-[var(--input-text)]" placeholder="laisvas tagas" value={tagValue} onChange={(e) => onTag(e.target.value)} />
      )}
    </span>
  )
}
  ...
  return (
    <div className="mt-5 border-t border-[var(--border-default)] pt-4">
      <div className="mb-2 flex items-center justify-between">
        ...
        {importing && <span className="text-[14px] text-[#ec4899]">{progress || 'Dirbama…'}</span>}
      </div>

      {/* Batch grupė importui/upload'ui */}
      <div className="mb-3 flex flex-wrap items-center gap-2 rounded-lg bg-[var(--bg-elevated)] px-3 py-2">
        <span className="text-[14px] font-semibold text-[var(--text-muted)]">Pridedamas naujas nuotraukas priskirti:</span>
      ...
      <div className="mb-3 rounded-lg border border-[var(--border-default)] p-3">
        ...
        {found.length > 0 && (
          <>
            <div className="mt-2 text-[14px] text-[var(--text-muted)]">Rasta {found.length} nuotraukų — importuojant bus perkeltos visos.</div>
      ...
      {sel.size > 0 && (
        <div className="mb-3 flex flex-wrap items-center gap-2 rounded-lg border border-[#ec4899]/40 bg-[#ec4899]/5 px-3 py-2">
          <span className="text-[14px] font-semibold text-[var(--text-primary)]">Pažymėta {sel.size} — priskirti grupei:</span>
          ...
          {photos.map((p) => {
            ...
            return (
              <div key={p.id} className={`group relative overflow-hidden rounded-lg border-2 ${selected ? 'border-[#ec4899]' : 'border-[var(--border-default)]'}`}>
                ...
                {gl && <span className="absolute inset-x-0 bottom-0 truncate bg-black/65 px-1.5 py-0.5 text-[12px] font-semibold text-white">{gl}</span>}
  ...
  return (
    <div className="rounded-xl border border-[var(--border-default)]">
      <div className="flex items-center gap-3 px-3 py-2.5">
        ...
        <div className="min-w-0 flex-1">
          <div className="truncate text-sm font-semibold text-[var(--text-primary)]">{p.name} {p.is_curated && <span className="ml-1 rounded bg-[#ec4899]/15 px-1.5 py-0.5 text-[12px] font-bold text-[#ec4899]">PUBLIC</span>}</div>
          <div className="truncate text-[14px] text-[var(--text-muted)]">{p.role_title || p.source || 'fotografas'}</div>
        </div>
