    ...
    <div className="overflow-hidden rounded-xl border border-[var(--input-border)] bg-[var(--bg-surface)] transition-all hover:border-[var(--border-strong)] hover:shadow-md">
      ...
      >
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          {card.hint && (
            <span className="truncate text-[14px] text-[var(--text-muted)]">{card.hint}</span>
          )}
        </div>
        {showBadge ? (
          <span className="shrink-0 rounded-full border border-orange-200 bg-orange-100 px-2 py-0.5 text-[12px] font-bold text-orange-700">
...
function SectionTitle({ icon, label, hint }: { icon: string; label: string; hint?: string }) {
  return (
    <div className="mb-3 flex items-baseline gap-2">
      ...
      {hint && <span className="text-[14px] text-[var(--text-faint)]">— {hint}</span>}
    </div>
  )
}
