...
type TopEntry = { pos: number; track_id: number; title: string; artist: string; cover_url: string | null; trend: string; wks?: number; slug?: string }
...
function Cover({ src, alt, size = 44, radius = 10 }: { src?: string | null; alt: string; size?: number; radius?: number }) {
  const h = strHue(alt)
  if (src) return <img src={src} alt={alt} style={{ width: size, height: size, borderRadius: radius, objectFit: 'cover', flexShrink: 0, display: 'block' }} />
  return <div style={{ width: size, height: size, borderRadius: radius, flexShrink: 0, background: `linear-gradient(135deg, hsl(${h},38%,16%), hsl(${(h+40)%360},28%,10%))`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: `hsl(${h},45%,45%)`, fontSize: size * 0.38, fontWeight: 800, fontFamily: 'Outfit, sans-serif' }}>{alt[0]?.toUpperCase() || '?'}</div>
}
...
function TrendIcon({ t }: { t: string }) {
  ...
  if (t === 'new') return <span style={{ fontSize: 8, fontWeight: 800, color: '#fbbf24', background: 'rgba(251,191,36,0.1)', padding: '1px 4px', borderRadius: 3 }}>N</span>
...
function SH({ label, href, cta = 'Visi →' }: { label: React.ReactNode; href?: string; cta?: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
      ...
      {href && <Link href={href} style={{ fontSize: 12, color: '#3d6094', fontWeight: 700, textDecoration: 'none' }}>{cta}</Link>}
    </div>
  )
}
...
function DienosDainaWidget() {
  ...
  useEffect(() => { fetch('/api/dienos-daina/nominations').then(r => r.json()).then(d => { setNoms(d.nominations || []); setLoading(false) }).catch(() => setLoading(false)) }, [])
  const w = noms[0]
  return (
    <div style={{ background: 'linear-gradient(145deg, rgba(22,55,140,0.22) 0%, rgba(8,13,22,0.98) 100%)', border: '1px solid rgba(30,80,200,0.18)', borderRadius: 16, overflow: 'hidden' }}>
      <div style={{ padding: '14px 16px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', gap: 12 }}>
        <Cover src={w?.tracks?.cover_url} alt={w?.tracks?.title || 'daina'} size={54} radius={10} />
        <div style={{ flex: 1, minWidth: 0 }}>
          <p style={{ fontSize: 10, color: '#2a4060', fontWeight: 700, margin: '0 0 2px', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Šiandien pirmauja</p>
          <h3 style={{ fontFamily: 'Outfit, sans-serif', fontSize: 15, fontWeight: 800, color: '#eef2fa', margin: '0 0 1px', lineHeight: 1.2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{loading ? '...' : w?.tracks?.title || 'Dar nėra'}</h3>
          <p style={{ fontSize: 11, color: 'rgba(180,200,235,0.45)', margin: 0 }}>{w?.tracks?.artists?.name || ''}</p>
        </div>
        <Link href="/dienos-daina" style={{ flexShrink: 0, background: '#f97316', color: '#fff', fontWeight: 800, fontSize: 11, padding: '7px 14px', borderRadius: 20, textDecoration: 'none', boxShadow: '0 3px 14px rgba(249,115,22,0.38)' }}>▶ Balsuoti</Link>
      </div>
      <div>
        <div style={{ padding: '8px 16px 6px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <Link href="/dienos-daina" style={{ fontSize: 9, color: '#3d6094', fontWeight: 700, textDecoration: 'none' }}>+ Siūlyti</Link>
        </div>
        {loading ? <div style={{ padding: '10px 16px', color: '#2a4060', fontSize: 12 }}>Kraunama...</div>
          : noms.slice(0, 5).map((n, i) => (
            <div key={n.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 16px', borderTop: '1px solid rgba(255,255,255,0.04)' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontSize: 11, fontWeight: 700, color: '#d0ddf0', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{n.tracks?.title}</p>
...
function ShoutboxWidget() {
  ...
  return (
    <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      <div style={{ padding: '11px 14px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <Link href="/bendruomene" style={{ fontSize: 10, color: '#3d6094', fontWeight: 700, textDecoration: 'none' }}>Visi →</Link>
      </div>
      <div>
      ...
      <div style={{ padding: '9px 12px', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
        <Link href="/bendruomene" style={{ display: 'block', textAlign: 'center', padding: '7px', borderRadius: 10, background: 'rgba(90,102,200,0.08)', border: '1px solid rgba(90,102,200,0.18)', color: '#7080c0', fontSize: 11, fontWeight: 700, textDecoration: 'none' }}>Prisijungti prie pokalbio →</Link>
      </div>
    </div>
  )
}
    ...
    <div className="hp-disc-grid">
      {discs.map(d => (
        <Link key={d.id} href={`/diskusijos/${d.slug}`}
          style={{ padding: '12px 14px', borderRadius: 12, background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)', textDecoration: 'none', display: 'block', transition: 'border-color 0.15s' }}
...
export default function Home() {
  ...
  const parseTop = (entries: any[]): TopEntry[] => entries.slice(0, 7).map(e => {
    ...
    return { pos: e.position, track_id: e.track_id, title: e.tracks?.title || '', artist: e.tracks?.artists?.name || '', cover_url: e.tracks?.cover_url || null, trend, wks: e.weeks_in_top, slug: e.tracks?.slug }
  })
  ...
  return (
    <>
      <style>{`
        ...
        .hp-gfilt{display:block;width:100%;padding:6px 10px;border-radius:8px;font-size:12px;font-weight:700;text-align:left;border:1px solid transparent;cursor:pointer;transition:all .15s;background:transparent;font-family:'DM Sans',sans-serif}
        .hp-gfilt:hover{color:#c0d0e8!important}
        ...
        @media(max-width:900px){
          ...
          .hp-hsub{font-size:14px!important;max-width:100%!important}
        ...
        @media(max-width:768px){
          ...
          .hp-ml{flex-direction:column!important}
          .hp-gsb{display:flex!important;flex-direction:row!important;width:auto!important;overflow-x:auto;scrollbar-width:none;padding-bottom:4px;gap:5px}
          .hp-gsb::-webkit-scrollbar{display:none}
          .hp-glbl{display:none!important}
          .hp-gfilt{display:inline-block!important;width:auto!important;white-space:nowrap;padding:5px 12px!important;border-radius:18px!important;border:1px solid rgba(255,255,255,.07)!important}
      ...
      <div className="hp">
        ...
        {hero && (
          <section style={{ position:'relative', overflow:'hidden', background:hero.bg, transition:'background .9s ease' }}>
            ...
            <div style={{ position:'relative', maxWidth:1360, margin:'0 auto', padding:'0 20px', display:'flex', alignItems:'stretch' }} className="hp-hl">
                  ...
                  <div className="hp-hnav" style={{ display:'flex', flexDirection:'column', gap:7, flexShrink:0 }}>
                    {['←','→'].map((ch,di) => (
                      <button key={ch} onClick={() => setHeroIdx(p => ((p+(di===0?-1:1))+heroSlides.length)%heroSlides.length)}
                        style={{ width:32, height:32, borderRadius:'50%', border:'1px solid rgba(255,255,255,.1)', background:'transparent', color:'rgba(180,200,235,.3)', cursor:'pointer', fontSize:13 }}>{ch}</button>
                    ))}
                  </div>
                )}
              </div>
              ...
              <div className="hp-hright" style={{ width:332, flexShrink:0, padding:'24px 0 24px 20px', display:'flex', flexDirection:'column' }}>
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:11 }}>
                  ...
                  <Link href="/topas" style={{ fontSize:11, color:'#3d6094', fontWeight:700, textDecoration:'none' }}>Visi →</Link>
                </div>
                <div style={{ flex:1 }}>
                  {chartData.length === 0
                    ...
                    : chartData.map((t,i) => (
                      <Link key={t.track_id||i} href={t.slug?`/muzika/${t.slug}`:'/topas'} className="hp-tr"
                        style={{ display:'flex', alignItems:'center', gap:9, padding:'6px 6px', borderRadius:7, cursor:'pointer', transition:'background .1s', textDecoration:'none', borderBottom:i<chartData.length-1?'1px solid rgba(255,255,255,.03)':'none' }}>
                        ...
                        {t.wks && <span style={{ fontSize:9, color:'#1a2a3c', flexShrink:0 }}>{t.wks}w</span>}
                      </Link>
                    ))}
                </div>
                <Link href="/topas" style={{ marginTop:10, display:'block', textAlign:'center', padding:'7px', borderRadius:10, border:'1px solid rgba(255,255,255,.07)', color:'#3d6094', fontSize:12, fontWeight:700, textDecoration:'none' }}
            ...
            {heroSlides.length > 1 && (
              <div style={{ position:'absolute', bottom:10, left:'50%', transform:'translateX(-160px)', display:'flex', gap:5, alignItems:'center' }}>
                {heroSlides.map((_,i) => (
                  <button key={i} onClick={() => setHeroIdx(i)}
                    style={{ borderRadius:3, border:'none', cursor:'pointer', padding:0, background:i===heroIdx?'#f97316':'rgba(255,255,255,.16)', width:i===heroIdx?18:5, height:5, transition:'all .25s' }} />
                ))}
              </div>
            )}
          </section>
        ...
        <div className="hp-cnt" style={{ maxWidth:1360, margin:'0 auto', padding:'42px 20px', display:'flex', flexDirection:'column', gap:48 }}>
          ...
          <section>
            <div className="hp-ml" style={{ display:'flex', gap:24, alignItems:'flex-start' }}>
              <div className="hp-gsb" style={{ width:106, flexShrink:0 }}>
                <p className="hp-glbl" style={{ fontSize:9, fontWeight:800, color:'#1a2a3c', textTransform:'uppercase', letterSpacing:'.12em', margin:'0 0 7px' }}>Stilius</p>
                {['Visi','Pop','Rokas','Hip-hop','Elektronika','Folk','Jazz'].map(g => (
                  <button key={g} className="hp-gfilt" onClick={() => setGenreFilter(g)}
                    style={{ color:genreFilter===g?'#90b8e8':'#2a4060', background:genreFilter===g?'rgba(29,78,216,.14)':'transparent', border:genreFilter===g?'1px solid rgba(29,78,216,.28)':'1px solid transparent' }}>{g}</button>
                ))}
              </div>
                ...
                <div>
                  ...
                  <div className="hp-scroll" style={{ display:'flex', gap:8, paddingBottom:2 }}>
                    {tracks.length===0 ? Array(8).fill(null).map((_,i) => (
                      ...
                      <Link key={t.id} href={`/muzika/${t.slug}`}
                        style={{ width:182, flexShrink:0, padding:'9px 11px', borderRadius:11, background:'rgba(255,255,255,.025)', border:'1px solid rgba(255,255,255,.07)', display:'flex', alignItems:'center', gap:9, textDecoration:'none', transition:'border-color .15s' }}
                        ...
                        <div style={{ flex:1, minWidth:0 }}>
                          <p style={{ fontFamily:'Outfit,sans-serif', fontSize:12, fontWeight:700, color:'#e0eaf8', margin:'0 0 2px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{t.title}</p>
                ...
                <div>
                  ...
                  <div className="hp-scroll" style={{ display:'flex', gap:9, paddingBottom:2 }}>
                    {albums.length===0 ? Array(5).fill(null).map((_,i) => (
                      ...
                      <Link key={a.id} href={`/muzika/${a.slug}`}
                        style={{ width:212, flexShrink:0, padding:'10px 12px', borderRadius:11, background:'rgba(255,255,255,.025)', border:'1px solid rgba(255,255,255,.07)', display:'flex', alignItems:'center', gap:10, textDecoration:'none', transition:'border-color .15s' }}
                        onMouseEnter={e=>(e.currentTarget.style.borderColor='rgba(255,255,255,.15)')}
                        onMouseLeave={e=>(e.currentTarget.style.borderColor='rgba(255,255,255,.07)')}>
                        <Cover src={a.cover_image_url} alt={a.title} size={46} radius={9} />
                        <div style={{ flex:1, minWidth:0 }}>
                          <p style={{ fontFamily:'Outfit,sans-serif', fontSize:13, fontWeight:700, color:'#e0eaf8', margin:'0 0 2px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{a.title}</p>
          ...
          <section><SH label="Bendruomenė" href="/diskusijos" cta="Visos diskusijos →" /><DiscussionsWidget /></section>
                  ...
                  : filtEvt.slice(0,5).map(ev => {
                    ...
                    return (
                      <Link key={ev.id} href={`/renginiai/${ev.slug}`}
                        style={{ display:'flex', alignItems:'center', gap:13, padding:'10px 14px', borderRadius:11, background:'rgba(255,255,255,.025)', border:'1px solid rgba(255,255,255,.07)', textDecoration:'none', transition:'border-color .15s' }}
                        onMouseEnter={e=>(e.currentTarget.style.borderColor='rgba(255,255,255,.14)')}
                        onMouseLeave={e=>(e.currentTarget.style.borderColor='rgba(255,255,255,.07)')}>
                        <div style={{ textAlign:'center', width:32, flexShrink:0 }}>
                          <p style={{ fontFamily:'Outfit,sans-serif', fontSize:19, fontWeight:900, color:'#f0f4fc', margin:0, lineHeight:1 }}>{d.getDate()}</p>
                        ...
                        <div style={{ flex:1, minWidth:0 }}>
                          <p style={{ fontFamily:'Outfit,sans-serif', fontSize:13, fontWeight:700, color:'#d0ddf0', margin:'0 0 2px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{ev.title}</p>
                          <p style={{ fontSize:11, color:'#2a4060', margin:0 }}>{ev.venues?.name||ev.venue_custom} · {ev.venues?.city}</p>
                        </div>
            ...
            <section>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:7 }}>
                {news.length===0 ? Array(4).fill(null).map((_,i) => (
                  ...
                  return (
                    <Link key={n.id} href={`/naujienos/${n.slug}`}
                      style={{ display:'flex', gap:11, padding:'10px 14px', borderRadius:11, background:'rgba(255,255,255,.025)', border:'1px solid rgba(255,255,255,.07)', textDecoration:'none', alignItems:'flex-start', transition:'border-color .15s' }}
                      onMouseEnter={e=>(e.currentTarget.style.borderColor='rgba(255,255,255,.14)')}
                      onMouseLeave={e=>(e.currentTarget.style.borderColor='rgba(255,255,255,.07)')}>
                      <div style={{ width:34, height:34, borderRadius:8, overflow:'hidden', flexShrink:0 }}><Cover src={n.image_small_url} alt={n.title} size={34} radius={8} /></div>
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ display:'flex', alignItems:'center', gap:5, marginBottom:3 }}>
                          {n.type && <span style={{ fontSize:9, fontWeight:800, padding:'2px 6px', borderRadius:4, background:`hsl(${h},35%,10%)`, color:`hsl(${h},55%,58%)`, border:`1px solid hsl(${h},35%,16%)` }}>{n.type}</span>}
                          <span style={{ fontSize:9, color:'#1a2a3c' }}>{timeAgo(n.published_at)}</span>
                        </div>
                        <p style={{ fontFamily:'Outfit,sans-serif', fontSize:13, fontWeight:600, color:'#b8ccde', margin:0, lineHeight:1.35, overflow:'hidden', display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical' } as any}>{n.title}</p>
                      </div>
                    </Link>
                  )
                })}
          ...
          <section>
            ...
            <div className="hp-ag" style={{ display:'grid', gridTemplateColumns:'repeat(6,1fr)', gap:18 }}>
              {artists.length===0 ? Array(12).fill(null).map((_,i) => (
                <div key={i} style={{ textAlign:'center' }}><Skel w={66} h={66} r={33} /><div style={{ margin:'8px auto 0', maxWidth:66 }}><Skel w="100%" h={9} /></div></div>
          ...
          <section>
            <div className="hp-cta" style={{ padding:'32px 40px', borderRadius:18, background:'linear-gradient(135deg,rgba(29,78,216,.09) 0%,rgba(255,255,255,.015) 100%)', border:'1px solid rgba(29,78,216,.15)', display:'flex', alignItems:'center', gap:22, position:'relative', overflow:'hidden' }}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                ...
                <p style={{ fontSize:13, color:'#2e4a62', margin:0, lineHeight:1.55, maxWidth:480 }}>Sukurk arba perimk savo profilį Music.lt platformoje. Skelk naujienas, renginius ir naują muziką tiesiai savo gerbėjams — nemokamai.</p>
              </div>
              <Link href="/atlikejai" className="hp-ctabtn"
                style={{ flexShrink:0, background:'#f97316', color:'#fff', fontWeight:800, fontSize:13, padding:'10px 24px', borderRadius:20, textDecoration:'none', boxShadow:'0 4px 16px rgba(249,115,22,.3)', whiteSpace:'nowrap', fontFamily:'Outfit,sans-serif', display:'inline-flex', alignItems:'center' }}>
                Pradėti nemokamai →
              </Link>
            </div>
          </section>
