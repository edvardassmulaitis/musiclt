...
export async function getArtistById(id: number): Promise<ArtistFull | null> {
  ...
  const [{ data: genreRows }, { data: linkRows }, { data: photoRows }, { data: breakRows }, { data: relatedRows }] =
    await Promise.all([
      ...
      supabase.from('artist_related').select('related_artist_id, artists!artist_related_related_artist_id_fkey(id, name)').eq('artist_id', id),
    ])
  ...
  return {
    ...
    related: (relatedRows || []).map((r: any) => ({ id: r.related_artist_id, name: r.artists?.name || '', yearFrom: '', yearTo: '' })),
  }
}
