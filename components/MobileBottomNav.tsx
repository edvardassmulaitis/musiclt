 ...
 *
 * v2 (5 vietos, centrinis „+" FAB), TIK IKONOS (be tekstų):
 *   🏠 Pradžia · ❤️ Sekami · ➕ Kurti · 📊 Topai · 💬 Pokalbiai
 *
 * Pagrindinis principas: Pradžia = muzikos pasaulis, ❤️ = mano muzikos pasaulis.
 * Pranešimai gyvena top bar'e (NotificationsBell). Pokalbiai išlaiko unread
 * badge'ą. Slepiasi desktop'e (CSS) ir /admin (render gate SiteShell'yje).
 */
  ...
  return (
    <>
      <style>{`
        ...
        .mbn-ico svg { width: 26px; height: 26px; }
        .mbn-badge {
      ...
      <nav className="mbn" aria-label="Apatinė navigacija">
        ...
        {/* Topai — stulpelinė diagrama (ranking) */}
        <Link href="/topai" className={`mbn-item${isTopai ? ' active' : ''}`} aria-label="Topai">
          <span className="mbn-ico">
            <svg viewBox="0 0 24 24" fill={isTopai ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="11" width="5" height="9" rx="1" /><rect x="9.5" y="4" width="5" height="16" rx="1" /><rect x="16" y="8" width="5" height="12" rx="1" /></svg>
          </span>
        </Link>
