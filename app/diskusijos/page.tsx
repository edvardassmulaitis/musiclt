    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 py-8 overflow-y-auto"
      ...
      <div className="w-full max-w-2xl rounded-3xl overflow-hidden shadow-2xl"
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-white/10">
          ...
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-full bg-white/5 text-gray-400 hover:text-white hover:bg-white/10 transition-all">✕</button>
        </div>

        <div className="p-6 space-y-4">
          <input type="text" value={title} onChange={e => setTitle(e.target.value)}
            placeholder="Pavadinimas..."
            className="w-full px-4 py-3 rounded-2xl text-white placeholder:text-gray-500 text-base font-semibold focus:outline-none focus:ring-2 focus:ring-blue-500/40"
            style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)' }} />

          <textarea value={text} onChange={e => setText(e.target.value)}
            placeholder="Parašyk diskusijos turinį..."
            rows={6}
            className="w-full px-4 py-3 rounded-2xl text-white placeholder:text-gray-500 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-blue-500/40"
            style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)' }} />

          {/* Tags */}
          <div>
            <p className="text-xs text-gray-500 font-bold uppercase tracking-wider mb-2">Tag'ai</p>
          ...
          <button onClick={submit} disabled={sending || title.trim().length < 5 || text.trim().length < 10}
            className="w-full py-3.5 rounded-2xl font-black text-white text-base transition-all hover:scale-[1.01] active:scale-[0.99] disabled:opacity-40"
...
function DiscussionCard({ d }: { d: Discussion }) {
  ...
  return (
    <Link href={`/diskusijos/${d.slug}`}
      className="block group transition-all hover:translate-y-[-1px]">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start gap-2 flex-wrap mb-1">
              ...
              {d.is_locked && <span className="text-[10px] font-black text-gray-600 uppercase">🔒</span>}
              {d.tags.map(tag => (
            ...
            <div className="flex items-center gap-3 mt-2">
              ...
              <span className="text-gray-800 text-xs">·</span>
              <span className="text-xs text-gray-600 sm:hidden">{d.comment_count} atl.</span>
    ...
    <div className="max-w-[860px] mx-auto px-5 py-10">
      ...
      <div className="flex items-center gap-3 mb-5 flex-wrap">
          ...
          {([
            ...
            <button key={k} onClick={() => setSort(k)}
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
                sort === k ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-white'
              }`}>
              {l}
            </button>
          ))}
        ...
        <div className="flex gap-2 flex-wrap">
          <button onClick={() => setActiveTag(null)}
            className={`px-3 py-1.5 rounded-full text-xs font-bold transition-all ${
              !activeTag ? 'bg-white/15 text-white' : 'bg-white/5 text-gray-500 border border-white/10 hover:bg-white/10'
            }`}>
            Visos
          </button>
          {AVAILABLE_TAGS.map(tag => (
            <button key={tag} onClick={() => setActiveTag(activeTag === tag ? null : tag)}
              className={`px-3 py-1.5 rounded-full text-xs font-bold transition-all ${
                activeTag === tag ? 'bg-indigo-500/30 text-indigo-300 border border-indigo-500/40' : 'bg-white/5 text-gray-500 border border-white/10 hover:bg-white/10'
              }`}>
              {tag}
            </button>
          ))}
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-xl font-black text-white mb-2">Dar nėra diskusijų</p>
          <p className="text-gray-500 mb-6">Būk pirmas — sukurk diskusijos temą!</p>
          {session && (
            <button onClick={() => setShowNew(true)}
