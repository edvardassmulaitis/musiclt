  ...
  return (
    <div className="sr-wrap">
      <style>{`
        ...
        /* Filtrų juosta — bendros .flt-* klasės (globals.css) */
      ...
      <div className="flt-bar flt-bar--between">
        <div className="flt-group" role="tablist">
          <button type="button" role="tab" aria-selected={mode === 'sekami'} className={`flt-chip${mode === 'sekami' ? ' on' : ''}`} onClick={() => switchMode('sekami')}>
            {IconHeart}<span>Mėgstami</span>
          </button>
          <button type="button" role="tab" aria-selected={mode === 'tau'} className={`flt-chip${mode === 'tau' ? ' on' : ''}`} onClick={() => switchMode('tau')}>
            {IconCompass}<span>Tau gali patikti</span>
          </button>
        </div>
        <Link href="/mano-muzika" className="flt-chip flt-chip--ico flt-gear" aria-label="Mano muzika — valdymas" title="Mano muzika — valdymas">{IconGear}</Link>
      </div>
