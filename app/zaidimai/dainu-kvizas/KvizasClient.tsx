...
//
// „Atspėk dainą" — songtrivia2.io stiliaus audio kvizas.
//
// Flow: kategorija → raundas (YT audio už overlay + 4 atsakymai + 15 s
// laikrodis) → reveal (matomas video + taškai) → ... → rezultatai (server-side
// užskaitymas per POST /api/zaidimai/kvizas).
//
// YT grojimas: plain <iframe autoplay=1 start=N> (site'o patikrintas pattern'as,
// žr. ChartYtPlayer). Kol neatsakyta — iframe pridengtas equalizer overlay'umi,
// kad nesimatytų pavadinimo. Jei mobile užblokavo autoplay — overlay'uje
// „Negirdi?" mygtukas remount'ina iframe su nauju user gesture.
...
export default function KvizasClient() {
  ...
  // Interval'as gyvena ilgiau nei render'io closure — timeout'o atveju
  // kviečiam VISADA šviežią answerRound per ref (kitaip stale round bug'as).
  ...
  function answerRound(answerId: number | null) {
    ...
    const points = correct ? 50 + Math.round(50 * (ROUND_MS - ms) / ROUND_MS) : 0
    setPicked(answerId)
  ...
  // answers state gali dar nebūti flush'intas kai submit'inam — laikom ref'ą
    ...
    <div className="kv-root">
      ...
      {phase === 'pick' && (
        <div className="kv-pick">
          ...
          <p className="kv-lead">Groja ištrauka — turi 15 sekundžių ir 4 variantus. Kuo greičiau atspėsi, tuo daugiau taškų. 10 raundų.</p>
          ...
          <p className="kv-note">Taškai skiriami už pirmus 3 kvizus per dieną. Prisijungę nariai gauna +50%.</p>
        </div>
      )}
          ...
          <div className="kv-player">
            ...
            {phase === 'reveal' && (
              <div className={`kv-verdict ${picked === round.correctId ? 'ok' : 'bad'}`}>
                {picked === round.correctId ? `+${lastPoints} tšk.` : picked === null ? 'Laikas baigėsi!' : 'Ne ta daina'}
              </div>
            )}
          </div>
      ...
      {phase === 'results' && (
        <div className="kv-results">
          <div className="kv-score-big" style={{ ['--acc' as any]: category.accent }}>
            ...
            <span className="kv-score-max">/ {result?.maxScore ?? (quiz?.rounds.length || 10) * 100} tšk.</span>
          </div>
          <p className="kv-result-line">
            Atspėta <b>{result?.correctCount ?? '—'}</b> iš {result?.roundCount ?? quiz?.rounds.length} · {category.label}
          </p>
          {result?.xp > 0 ? (
            ...
            <p className="kv-xp-line dim">Dienos taškų limitas išnaudotas — šis buvo treniruotė 💪</p>
          ) : null}
          {result?.xpRunsLeft > 0 && (
            <p className="kv-note">Šiandien taškai dar už {result.xpRunsLeft} kviz{result.xpRunsLeft === 1 ? 'ą' : 'us'}.</p>
          )}
          <div className="kv-result-actions">
            <button className="kv-again" onClick={() => startQuiz(category)}>Žaisti dar kartą</button>
...
.kv-top { display: flex; align-items: center; justify-content: space-between; margin-bottom: 18px; }
...
.kv-result-line { font-size: 16px; color: var(--text-secondary); margin: 6px 0; }
.kv-result-line b { color: var(--text-primary); }
.kv-xp-line { font-size: 16px; font-weight: 700; color: #f59e0b; margin: 6px 0; }
.kv-xp-line.dim { color: var(--text-muted); font-weight: 500; }
.kv-result-actions { display: flex; gap: 10px; margin: 22px 0 14px; flex-wrap: wrap; justify-content: center; }
.kv-again {
  font-size: 16px; font-weight: 800; color: #fff; cursor: pointer; border: 0; border-radius: 999px; padding: 12px 26px;
  background: linear-gradient(135deg, #f59e0b, #f97316); box-shadow: 0 10px 26px rgba(245,158,11,0.35);
}
.kv-other {
  font-size: 16px; font-weight: 800; color: var(--text-primary); cursor: pointer; border-radius: 999px; padding: 12px 26px;
